...
def chord(ta: RpcSubprocess, key: str, **mods: bool) -> bool:
    out = ta.invoke("/external/key", {"key": key, **mods})
    time.sleep(SETTLE)
    return out
...
def body() -> None:
    with RpcSubprocess("hello-textarea", request_timeout=12.0) as ta:
        ...
        assert_eq(caret(ta), len(SEED), "Ctrl+End moves to the document end")
        type_text(ta, "wow")
        assert_eq(text(ta), SEED + "wow", "typing appended a word")
        assert_eq(caret(ta), len(SEED) + 3, "caret at the new end")

        assert_eq(chord(ta, "z", ctrl=True), True, "Ctrl+Z recognized")
        assert_eq(text(ta), SEED, "one undo removed the whole 'wow' run (coalescing)")
        ...
        assert_eq(text(ta), SEED + "wow", "Ctrl+Shift+Z redid the typing")
        chord(ta, "z", ctrl=True)
        assert_eq(text(ta), SEED, "undo again before testing Ctrl+Y")
        assert_eq(chord(ta, "y", ctrl=True), True, "Ctrl+Y recognized")
        assert_eq(text(ta), SEED + "wow", "Ctrl+Y is the alternate redo")
        chord(ta, "z", ctrl=True)
        assert_eq(text(ta), SEED, "back to the floor for the next phase")
        ...
        assert_eq(text(ta), " line\nsecond line\nthird line", "the red word is gone")
        ...
        assert_eq(text(ta), SEED, "undo restored the deleted word")
        wait_until(
