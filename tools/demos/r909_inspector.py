...
def _assert_boot_schema(tf: RpcSubprocess, gp) -> None:
...
def _assert_player_schema(tf: RpcSubprocess, gp) -> None:
    """(B) Player's typed property schema: the common actor base
    (Visible / Layer / Locked) then its type-specific tail."""
    assert _q(tf, gp.at("name", i=0)) == "Visible" and _q(tf, gp.at("kind", i=0)) == "bool"
    assert _q(tf, gp.at("value", i=0)) is True, "Visible seed True"
    assert _q(tf, gp.at("name", i=1)) == "Layer" and _q(tf, gp.at("kind", i=1)) == "int"
    assert _q(tf, gp.at("value", i=1)) == 1, "Layer seed 1"
    assert _q(tf, gp.at("name", i=2)) == "Locked" and _q(tf, gp.at("kind", i=2)) == "bool"
    assert _q(tf, gp.at("name", i=3)) == "Health" and _q(tf, gp.at("kind", i=3)) == "int"
    assert _q(tf, gp.at("value", i=3)) == 100, "Health seed 100"
    assert _q(tf, gp.at("name", i=4)) == "Speed" and _q(tf, gp.at("kind", i=4)) == "float"
    assert abs(float(_q(tf, gp.at("value", i=4))) - 6.5) < 1e-9, "Speed seed 6.5"
    assert _q(tf, gp.at("name", i=5)) == "Team" and _q(tf, gp.at("kind", i=5)) == "choice"
    team = _q(tf, gp.at("value", i=5))
    ...
    assert _q(tf, gp.at("name", i=6)) == "Tint" and _q(tf, gp.at("kind", i=6)) == "color"
    tint = _q(tf, gp.at("value", i=6))
    assert isinstance(tint, dict) and (tint["r"], tint["g"], tint["b"]) == (0x4F, 0x9D, 0xFF), (
        f"Tint seed channels; got {tint!r}"
    )


def _assert_select_retargets(tf: RpcSubprocess, gp) -> None:
    ...
    assert _q(tf, gp.at("name", i=0)) == "Visible", "Camera's first property is the common base"
    assert _q(tf, gp.at("name", i=3)) == "Field of View"
    assert abs(float(_q(tf, gp.at("value", i=3))) - 60.0) < 1e-9, "Camera FoV seed"


def _assert_intervene_selected(tf: RpcSubprocess, gp) -> None:
    ...
    assert _q(tf, gp.at("name", i=3)) == "Intensity"
    # back to Player for the editing section.
    _select(tf, 0)
    assert _q(tf, "selected") == 0


def _assert_typed_editing(tf: RpcSubprocess, gp) -> None:
    """(E) int + bool editing of the selected (Player) object."""
    tf.intervene(f"/external/{gp.at('value', i=3)}", 42)  # Health int.
    assert _q(tf, gp.at("value", i=3)) == 42, "Health edited to 42"
    tf.intervene(f"/external/{gp.at('value', i=0)}", False)  # Visible bool.
    assert _q(tf, gp.at("value", i=0)) is False, "Visible toggled off"


def _assert_choice_and_color(tf: RpcSubprocess, gp) -> None:
    """(F) Choice (option index) + Color (hex) edits via with_intervene."""
    tf.intervene(f"/external/{gp.at('value', i=5)}", 1)  # Team -> Blue.
    team = _q(tf, gp.at("value", i=5))
    assert team["selected"] == 1 and team["label"] == "Blue", f"Team -> Blue; got {team!r}"
    tf.intervene(f"/external/{gp.at('value', i=6)}", "#ff0000")  # Tint -> red.
    tint = _q(tf, gp.at("value", i=6))
    assert (tint["r"], tint["g"], tint["b"]) == (0xFF, 0x00, 0x00), f"Tint -> red; got {tint!r}"


def _assert_per_object_isolation(tf: RpcSubprocess, gp) -> None:
    """(G) edits on the single-selected Player do not leak to Camera, and
    persist across a re-select (per-object state)."""
    _select(tf, 1)  # Camera.
    assert abs(float(_q(tf, gp.at("value", i=3))) - 60.0) < 1e-9, "Camera FoV untouched by Player edits"
    _select(tf, 0)  # back to Player.
    assert _q(tf, gp.at("value", i=3)) == 42, "Player Health edit persisted"
    assert _q(tf, gp.at("value", i=0)) is False, "Player Visible edit persisted"
    team = _q(tf, gp.at("value", i=5))
...
def body() -> None:
    with RpcSubprocess("hello-inspector", boot_grace=1.5) as tf:
        ...
        _assert_boot_schema(tf, gp)          # (A)
        _assert_player_schema(tf, gp)        # (B)
        _assert_select_retargets(tf, gp)     # (C)
        _assert_intervene_selected(tf, gp)   # (D)
        _assert_typed_editing(tf, gp)        # (E)
        _assert_choice_and_color(tf, gp)     # (F)
        _assert_per_object_isolation(tf, gp) # (G)
        _assert_errors(tf)               # (H)
