...
def body() -> None:
    with RpcSubprocess(EXAMPLE, boot_grace=1.5) as tf:
        ...
        time.sleep(PAUSE)
        assert_eq(tf.request("focus/get").result.get("focused"), DIR, "the list owns shell focus")
        snap = tf.snapshot(source="paint", viewport=WIN)
        ...
        time.sleep(PAUSE)
        assert_eq(cursor(tf), 1, "ArrowDown advances the cursor")
        tf.key(path=DIR, name="ArrowDown")
        tf.key(path=DIR, name="ArrowDown")
        time.sleep(PAUSE)
        assert_eq(cursor(tf), 3, "two more ArrowDowns reach the last /proj row")
        tf.key(path=DIR, name="ArrowDown")
        time.sleep(PAUSE)
        ...
        time.sleep(PAUSE)
        assert_eq(cursor(tf), 2, "ArrowUp steps back")
        tf.key(path=DIR, name="Home")
        time.sleep(PAUSE)
        assert_eq(cursor(tf), 0, "Home jumps to the first row")
        tf.key(path=DIR, name="ArrowUp")
        time.sleep(PAUSE)
        assert_eq(cursor(tf), 0, "ArrowUp at the top clamps (no wrap)")
        tf.key(path=DIR, name="End")
        time.sleep(PAUSE)
        assert_eq(cursor(tf), 3, "End jumps to the last row")
        ...
        time.sleep(PAUSE)
        assert_eq(tf.query(dpath("cwd")), "/proj/src", "Enter on a folder navigates in")
        ...
        tf.key(path=DIR, name="End")
        time.sleep(PAUSE)
        assert_eq(cursor(tf), src_count - 1, "End moves to the last src row")
        snap = tf.snapshot(source="paint", viewport=WIN)
        ...
        time.sleep(PAUSE)
        snap = tf.snapshot(source="paint", viewport=WIN)
        assert 0 in present_rows(snap), "Home scrolled the cursor back to the top"

        # Enter on a file row picks it (src row 0 is a file — lib.rs).
        tf.key(path=DIR, name="Enter")
        time.sleep(PAUSE)
        assert_eq(
            tf.query(dpath("selected")),
            "/proj/src/lib.rs",
            "Enter on a file row picks it",
        )
        ...
        time.sleep(PAUSE)
        assert_eq(cursor(tf), 2, "intervene moved the cursor (aria-activedescendant write)")
        snap = tf.snapshot(source="paint", viewport=WIN)
        assert ring_row_aligned(snap, 2), "the ring follows an RPC-set cursor"
        tf.intervene(dpath("cursor"), None)
        time.sleep(PAUSE)
        assert_eq(cursor(tf), 0, "intervene Null resets the cursor to the first row")
        ...
        assert_eq(
            tf.query(dpath("selected")),
            "/proj/Cargo.toml",
            "a pointer click still selects, alongside the keyboard cursor",
        )
