...
def pos(tf, gp, nid: int) -> tuple[int, int]:
    return (
...
def body() -> None:
    with RpcSubprocess(EXAMPLE, boot_grace=1.5) as tf:
        ...
        moved = wait_until(
            lambda: pos(tf, gp, 0) if pos(tf, gp, 0) != before else None,
        ...
        assert_eq(pos(tf, gp, 0), before, "undo reverted the drag to the grab position")
        assert_eq(redo(tf), True, "redo the drag")
        assert_eq(pos(tf, gp, 0), moved, "redo re-applied the drag")
        ...
        tf.key(path=G, name="ArrowRight")
        after = wait_until(
            lambda: pos(tf, gp, 0) if pos(tf, gp, 0)[0] >= bx + 3 * 12 else None,
        ...
        assert_eq(pos(tf, gp, new_id), add_pos, "node back at its add-time position")
        assert_eq(redo(tf), True, "redo the move")
        assert_eq(pos(tf, gp, new_id), moved_pos, "redo restores the moved position (R851 wrinkle fixed)")
