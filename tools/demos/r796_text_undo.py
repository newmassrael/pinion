...
from rpc_verify import RpcSubprocess, assert_eq, run_demo, wait_query  # noqa: E402
...
def chord(tf: RpcSubprocess, key: str, **mods: bool) -> bool:
    ...
    return tf.invoke("/external/key", payload)
...
def body() -> None:
    with RpcSubprocess("hello-textfield") as tf:
        ...
        wait_query(tf, "/external/state", "Focused", desc="field focused")
        ...
        wait_query(
            ...
            desc="one undo removed the entire 'world' run, not a single char",
        )
        assert_eq(caret(tf), 6, "caret restored to the word boundary")

        # ── (C) a second Ctrl+Z drops the first word + space ────────
        chord(tf, "z", ctrl=True)
        wait_query(tf, "/external/text", "",
                   desc="second undo cleared the first word + trailing space")
        assert_eq(caret(tf), 0, "caret back to the start")
        # Nothing left to undo.
        assert_eq(chord(tf, "z", ctrl=True), True, "Ctrl+Z consumed even at the bottom")
        wait_query(tf, "/external/text", "", desc="no further change at the history bottom")

        # ── (D) redo via Ctrl+Y and via Ctrl+Shift+Z ────────────────
        assert_eq(chord(tf, "y", ctrl=True), True, "Ctrl+Y redo recognized")
        wait_query(tf, "/external/text", "hello ", desc="Ctrl+Y re-applied the first step")
        assert_eq(chord(tf, "z", ctrl=True, shift=True), True, "Ctrl+Shift+Z recognized")
        wait_query(tf, "/external/text", "hello world",
                   desc="Ctrl+Shift+Z re-applied the second step")

        # ── (F) a Backspace is its own undo step ────────────────────
        chord(tf, "Backspace")
        wait_query(tf, "/external/text", "hello worl", desc="backspace removed the last char")
        chord(tf, "z", ctrl=True)
        wait_query(
            tf, "/external/text", "hello world",
            desc="undo restored the backspace alone (delete is a separate group)",
        )
