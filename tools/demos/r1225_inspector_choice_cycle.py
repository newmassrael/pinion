...
def team(tf: RpcSubprocess, gp) -> int:
    """The representative Team option index off the Team row's Choice JSON."""
    return q(tf, gp.at("value", i=TEAM))["selected"]


def wait_team(tf: RpcSubprocess, gp, expected: int, desc: str) -> None:
    wait_until(lambda: True if team(tf, gp) == expected else None, desc=desc)
...
def body() -> None:
    with RpcSubprocess("hello-inspector", boot_grace=1.5) as tf:
        ...
        assert_eq(q(tf, gp.at("kind", i=1)), "int", "row 1 (Layer) is an Int")
        assert_eq(q(tf, gp.at("kind", i=TEAM)), "choice", "row 5 (Team) is a Choice")
        assert_eq(q(tf, gp.at("name", i=TEAM)), "Team", "row 5 is named Team")
        assert_eq(q(tf, gp.at("mixed", i=TEAM)), False, "single selection -> Team not mixed")
        assert_eq(q(tf, gp.at("value", i=TEAM))["options"],
                  ["Red", "Blue", "Neutral"], "Team's option list")
        assert_eq(team(tf, gp), 0, "Team starts on Red (option 0)")

        # ── (B) RPC cycle wraps forward + backward ───────────────────
        assert_eq(cycle(tf, f"{TEAM},1"), True, "cycle +1 on a Choice returns true")
        wait_team(tf, gp,1, desc="Red -> Blue")
        cycle(tf, f"{TEAM},1")
        wait_team(tf, gp,2, desc="Blue -> Neutral")
        cycle(tf, f"{TEAM},1")
        wait_team(tf, gp,0, desc="Neutral wraps forward to Red")
        cycle(tf, f"{TEAM},-1")
        wait_team(tf, gp,2, desc="Red wraps backward to Neutral")
        cycle(tf, f"{TEAM},-1")
        wait_team(tf, gp,1, desc="Neutral -> Blue backward")
        ...
        wait_team(tf, gp,(before + 1) % 3, desc="ArrowRight cycles the Choice +1")
        ...
        wait_team(tf, gp,before, desc="ArrowLeft cycles the Choice -1 (back)")
        ...
        wait_team(tf, gp,(before + 1) % 3, desc="clicking the Choice cell cycles +1")
