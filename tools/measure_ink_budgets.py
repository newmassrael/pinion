...
def measure(example: str) -> tuple[object, object, str]:
    """`(containment, smear, reason)` for one example.

    Either count is [`UNMEASURED`] when the surface could not be booted or the
    ...
    try:
        with RpcSubprocess(example) as app:
            try:
                ...
                return UNMEASURED, UNMEASURED, f"scene/containment: {exc}"[:120]
            try:
                runs = app.request("scene/text_painted").result.get("runs", [])
            except Exception as exc:  # noqa: BLE001
                return escapes, UNMEASURED, f"scene/text_painted: {exc}"[:120]
            return escapes, _smear_pairs(runs), ""
    except AssertionError as exc:
        # A boot gate other than these two refused (pointer-reach, for
        # instance). The surface booted, so this is a real defect on it and NOT
        # an unmeasured one — but this tool cannot read past the refusal.
        return UNMEASURED, UNMEASURED, f"a boot gate refused: {exc}"[:120]
    except Exception as exc:  # noqa: BLE001
        return UNMEASURED, UNMEASURED, f"{type(exc).__name__}: {exc}"[:120]
...
def main() -> int:
    ...
    for i, example in enumerate(targets, 1):
        c, s, reason = measure(example)
        cont.append((example, c, reason))
        smear.append((example, s, reason))
        print(f"[{i}/{len(targets)}] {example}: containment={c} smear={s} {reason}",
    ...
    print(f"wrote {CONTAINMENT} and {SMEAR}", file=sys.stderr)
    return 0
