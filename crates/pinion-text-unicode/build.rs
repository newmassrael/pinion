...
fn main() {
    ...
    emit_bidi_tables(&bidi_path, &bidi_ranges);
}
...
/// Emit `BIDI_CLASS_RANGES: &[(u32, u32, u8)]` to `bidi_tables.rs`.
/// The runtime crate `include!`s this file inside its `bidi` module
/// and re-derives the `BidiClass` enum from the `u8` indices via a
/// const match.
fn emit_bidi_tables(out_path: &Path, ranges: &[(u32, u32, u8)]) {
    use std::fmt::Write as _;
    let mut s = String::new();
    s.push_str("// AUTO-GENERATED by pinion-text-unicode/build.rs from\n");
    s.push_str("// ucd/DerivedBidiClass.txt (UCD 16.0.0). Do not edit by hand.\n\n");
    let _ = writeln!(
        s,
