...
fn main() {
    ...
    emit_tables(&out_path, &parsed, &exclusions, &primary_composites);
}
...
fn emit_decomp_table(
    ...
    writeln!(s, "pub(crate) const {name}: &[(u32, &[u32])] = &[")
...
fn emit_ccc_table(s: &mut String, ccc: &[(u32, u8)]) {
    ...
    s.push_str(
        "pub(crate) const CANONICAL_COMBINING_CLASS: &[(u32, u8)] = &[\n",
    );
    for (cp, value) in ccc {
...
fn emit_exclusion_table(s: &mut String, exclusions: &[u32]) {
    ...
    s.push_str("pub(crate) const FULL_COMPOSITION_EXCLUSION: &[u32] = &[\n");
...
fn emit_primary_composites_table(
    ...
    s.push_str(
        "pub(crate) const PRIMARY_COMPOSITES: &[((u32, u32), u32)] = &[\n",
    );
