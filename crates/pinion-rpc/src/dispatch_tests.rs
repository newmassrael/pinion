...
#[test]
fn r1433_scene_rotation_gesture_enqueues_rotation_and_phase() {
    let mut scene = counted_scene(0);
    let previews = PreviewLedger::default();
    let revision = SceneRevision::default();
    let mut inbox: Vec<DeferredInput> = Vec::new();
    let mut ctx =
        DispatchContext::new(&mut scene, &previews, &revision).with_deferred_inputs(&mut inbox);
    let req = r#"{"jsonrpc":"2.0","method":"scene/rotation_gesture","params":{"at":{"x":120.0,"y":90.0},"rotation":15.0,"phase":"update"},"id":301}"#;
    let resp = parse_response(&dispatch(&mut ctx, req).unwrap());
    assert!(resp.error.is_none(), "{:?}", resp.error);
    assert_eq!(resp.result, Some(Value::Null));
    assert_eq!(inbox.len(), 1);
    let DeferredInput::RotationGesture {
        x,
        y,
        rotation,
        phase,
        ...
        panic!("expected RotationGesture variant, got {:?}", inbox[0]);
    };
    assert!((x - 120.0).abs() < f64::EPSILON);
    assert!((y - 90.0).abs() < f64::EPSILON);
...
#[test]
fn r1433_scene_rotation_gesture_rejects_bad_phase_and_missing_fields() {
    // Every malformed shape rejects invalid_params (-32602) and enqueues
    // nothing, so a typo surfaces at the call instead of a silent default.
    for (params, why) in [
        (
            r#"{"at":{"x":10.0,"y":10.0},"rotation":10.0,"phase":"started"}"#,
            "unknown phase",
        ),
        (
            r#"{"at":{"x":10.0,"y":10.0},"rotation":10.0,"phase":42}"#,
            "non-string phase",
        ),
        (
            r#"{"at":{"x":10.0,"y":10.0},"phase":"begin"}"#,
        ...
        (
            r#"{"at":{"x":10.0,"y":10.0},"rotation":10.0}"#,
            "missing phase",
        ),
    ] {
        let mut scene = counted_scene(0);
        ...
        let mut ctx =
            DispatchContext::new(&mut scene, &previews, &revision).with_deferred_inputs(&mut inbox);
        let req = format!(
            r#"{{"jsonrpc":"2.0","method":"scene/rotation_gesture","params":{params},"id":1}}"#
        );
        let resp = parse_response(&dispatch(&mut ctx, &req).unwrap());
        assert!(resp.error.is_some(), "{why} should reject");
        assert_eq!(resp.error.unwrap().code, -32602, "{why}");
