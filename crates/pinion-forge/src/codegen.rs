...
/// Emits a self-contained Rust module wrapping the owned
/// `pinion_gpu::GpuContext` and `pinion_gpu::GpuSurface` together with
/// `vello::Renderer` in a concrete type — zero virtual dispatch per
/// §5.16 R45 (build-time codegen per target).
/// Vello 0.9 canonical pattern (Xilem reference impl):
/// `render_to_texture` → blit → present. Async `new` because wgpu
/// adapter+device acquisition is async; called once at app boot per §6.3
/// boundary.
///
/// R1537 — the device moved out of `vello::util::RenderContext`, whose
/// private constructor fixes the feature set and so can never yield a
/// device that answers how long the GPU took. The emitted renderer now
/// brackets each frame with two timestamp queries.
///
...
/// `kind="renderer" backend="vello"` manifest entry. Wraps the owned
/// `pinion_gpu::GpuContext` and `pinion_gpu::GpuSurface` together with
/// Vello's `Renderer` in a single concrete type so callers see zero
/// virtual dispatch (§5.16 R45, compile-time per target).
///
...
/// implementation detail. Keep the macro's field list in sync. R1537
/// added `frame_timer` to that contract for the same reason.
pub struct __NAME__ {
    context: ::pinion_gpu::GpuContext,
    surface: ::pinion_gpu::GpuSurface,
    renderer: ::vello::Renderer,
    /// R1537 §5.16 — the GPU-side frame clock, or `None` on a host whose
    /// adapter does not offer timestamp queries.
    ///
    /// Owned by the renderer because the two timestamps have to bracket
    /// *this* method's submissions: the opening one is ordered before the
    /// rasterizer's internal submit, the closing one rides the blit
    /// encoder, and no caller can reach between those.
    frame_timer: ::std::option::Option<::pinion_gpu::FrameTimer>,
...
pub enum __ERR_NAME__ {
    ...
    /// happens strictly *before* a rasterizer exists: there is no vello
    /// error to report when the failure is that the host has no GPU.
    Gpu(::pinion_gpu::GpuError),
}
...
impl ::std::fmt::Display for __ERR_NAME__ {
    fn fmt(&self, f: &mut ::std::fmt::Formatter<'_>) -> ::std::fmt::Result {
        match self {
            ...
            Self::Gpu(e) => write!(f, "gpu context init failed: {e}"),
        }
    }
}
...
impl ::std::error::Error for __ERR_NAME__ {
    fn source(&self) -> ::std::option::Option<&(dyn ::std::error::Error + 'static)> {
        match self {
            ...
            Self::Gpu(e) => ::std::option::Option::Some(e),
        }
    }
}
...
impl ::std::convert::From<::pinion_gpu::GpuError> for __ERR_NAME__ {
    fn from(e: ::pinion_gpu::GpuError) -> Self {
        Self::Gpu(e)
    }
}
...
impl __NAME__ {
        ...
        W: ::std::convert::Into<::vello::wgpu::SurfaceTarget<'static>>,
    {
        ...
        // set, so no consumer of it can ask for TIMESTAMP_QUERY and no
        // frame rendered on it can state what the GPU took. The R1060
        // COPY_SRC negotiation and the surface's intermediate target moved
        // into `pinion_gpu::GpuSurface` with it.
        let (context, surface) = ::pinion_gpu::GpuContext::new(
            target,
            width,
            height,
        ...
        context.on_uncaptured_error(|error| {
            ::std::eprintln!(
        ...
        // `None` on a host whose adapter has no timestamp queries — the
        // window renders, and reports no GPU time rather than a zero.
        let frame_timer = ::pinion_gpu::FrameTimer::new(context.device(), context.queue());
    ...
    /// R1537 §5.16 — what this renderer can say about the GPU's own clock.
    ///
    /// Three states, not an `Option<u64>`: a host with no timestamp
    /// queries (`Unsupported`) and a window whose first measurement is
    /// still in flight (`Pending`) are different facts that call for
    /// different responses, and one `None` cannot carry both. The absence
    /// of a timer IS the unsupported case — `pinion_gpu::FrameTimer`
    /// cannot be constructed on a device that lacks the features.
    ///
    /// `Measured` is one frame behind by construction; see
    /// `pinion_gpu::FrameTimer`.
    pub fn gpu_clock(&mut self) -> ::pinion_gpu::GpuFrameClock {
        self.frame_timer
            .as_mut()
    ...
    pub fn render(
        ...
        // R1537 §5.16 — harvest whatever the GPU has finished reporting
        // before starting a new measurement, and open this frame's span.
        // The opening timestamp must be ORDERED BEFORE the rasterizer's
        // work, and `render_to_texture` submits internally rather than
        // recording into an encoder this method holds, so there is no
        // encoder in existence at that point to append to — hence a
        // separate, near-empty command buffer. It is the whole cost of the
        // measurement, and it is paid only on a host that can measure.
        if let ::std::option::Option::Some(timer) = self.frame_timer.as_mut() {
            timer.collect(self.context.device());
            let mut open = self.context.device().create_command_encoder(
                &::vello::wgpu::CommandEncoderDescriptor {
                    label: ::std::option::Option::Some("pinion frame timer open"),
                },
            );
            if timer.begin(&mut open) {
                self.context.queue().submit([open.finish()]);
            }
        }
        ...
        // R1537 §5.16 — close the span on the encoder that already exists.
        // Everything the GPU does for this frame — the rasterizer's compute
        // passes and this blit — lies between the two timestamps.
        if let ::std::option::Option::Some(timer) = self.frame_timer.as_mut() {
            timer.end(&mut encoder);
        }
        self.context.queue().submit([encoder.finish()]);
        // Only now can the staging buffer be asked for: a map waits on
        // work that has to have been submitted.
        if let ::std::option::Option::Some(timer) = self.frame_timer.as_mut() {
            timer.after_submit();
        }
