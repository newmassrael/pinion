...
//
// Regular `//` line comments rather than `//!` inner-doc per R46.3 —
// callers commonly include!() this file inside `mod { ... }`, where
// `//!` at the top would be parsed as the parent module's inner doc
// and rejected when it precedes the codegen's `use vello::...` items.
