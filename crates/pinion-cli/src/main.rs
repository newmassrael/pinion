...
//! lands the `design-verify` sub-command that drives the
...
// verify` instead of `pinion design-verify`), an externally-visible
// CLI surface break for a stylistic gain. Allow with the rationale
// pinned to the variant set.
#[allow(
    clippy::enum_variant_names,
    reason = "clap derives the sub-command CLI name from the variant id; \
              stripping the shared prefix would rename every one of them, an \
              externally-visible CLI surface break for a stylistic gain"
)]
enum Command {
    ...
    /// (`examples/design-button-m3`).
    DesignVerify(design_verify::DesignVerifyArgs),
    ...
    DesignFetchImage(design_image::DesignImageArgs),
    ...
    /// verification loop.
    DesignDiff(design_diff::DesignDiffArgs),
}

fn main() {
    let cli = Cli::parse();
    let result = match &cli.command {
        Command::DesignVerify(args) => design_verify::run(args),
        Command::DesignFetchImage(args) => design_image::run(args),
        Command::DesignDiff(args) => design_diff::run(args),
    };
