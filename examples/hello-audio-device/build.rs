//! §5.16 pinion-forge codegen entrypoint for `hello-audio-device`.
//! Mirrors `examples/hello-audio-rt/build.rs`: compiles `app.pinion.xml` to
//! `$OUT_DIR/app.rs`, which `main.rs` pulls in via `include!`. On DSL-level
//! failure every diagnostic prints on its own line before the build aborts.
