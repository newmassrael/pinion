//! `hello-syntax-highlight` — R904 §5.22 §5.36 syntax highlighting consumer
//! for the `TextField` widget.
//!
//! ## What it demonstrates
//!
//! The depth slice that turns a text field into a **code editor**: the styled
//! runs are *derived from the buffer content*, not applied by hand. The binding
//! attaches a highlighter once —
//! [`TextEditState::attach_highlighter`](pinion_core::widgets::text_edit::TextEditState::attach_highlighter)
//! wrapping [`pinion_core::syntax::highlight_code`] over a C-family keyword set
//! — and from then on
//! [`style_runs`](pinion_core::widgets::text_edit::TextEditState::style_runs)
//! re-derives the colouring from the live text on every read (the
//! `find_matches` re-derive shape). Because that one accessor is the single
//! site paint, caret geometry, hit-test, and the `scene/style_runs` RPC all
//! call, every consumer sees the syntax colouring with no per-consumer change —
//! and an AI agent reads the editor's token colouring directly over RPC.
//!
//! The seam is a closure, not a framework `Language` trait: the grammar lives
//! in the caller's tokeniser ([[abstraction-needs-second-consumer]]); a second
//! language is a sibling free fn behind the same `attach_highlighter`.
//!
//! ## Architecture
//!
//! - State shape: `(TextFieldState, u32)` — interaction state + caret byte
//!   offset. Text + the highlighter live on the reactive [`TextEditState`]
//!   reached via `use_text_edit_state(TF_TAG)`.
//! - Plain editing (type, Backspace, Arrow, Ctrl+Z) reuses the R56 substrate
//!   unchanged; only the derived colouring is new. Typing a keyword re-colours
//!   it immediately because the derive re-runs on the text-change repaint.
//!
//! ## Try it
//!
//! ```text
//! cargo run --release -p hello-syntax-highlight
...
// pinion-forge codegen output. Defines `pub struct HelloSyntaxRenderer`
...
vello_renderer_impl!(HelloSyntaxRenderer, HelloSyntaxRendererError);
...
/// R904 §5.36 — the seeded line of code. One line carries all four token
/// classes the highlighter recognises: the `let` keyword, the `42` number, the
/// `"x"` string, and the `// sum` line comment.
const SEED_CODE: &str = "let n = 42 + \"x\" // sum";

/// R904 §5.36 — the C-family keyword set the highlighter colours. The
/// configurable slice IS the language seam — a different dialect swaps the set,
/// no new substrate ([[abstraction-needs-second-consumer]]).
const KEYWORDS: &[&str] = &[
    "let", "fn", "mut", "if", "else", "for", "while", "return", "struct", "impl", "pub", "use",
    "match", "true", "false",
];

/// R904 §5.36 — tag for the status container so the demo reads the
/// "{n} tokens highlighted" line as scene-as-data (`find_by_tag`).
const STATUS_TAG: &str = "syntax_status";
...
/// 1. `"Syntax Highlighting"` title label (18 px white).
/// 2. The code field: 360×40, `tag = "code_editor"`. The seeded code paints
///    with the highlighter-derived styled runs (keyword / string / comment /
///    number colours) plus the caret overlay.
/// 3. Status bar (`tag = "syntax_status"`) — "{n} tokens highlighted", derived
///    from the same `style_runs()` the field paints (the visible mirror of the
///    `scene/style_runs` RPC).
#[allow(
...
fn view(state: (TextFieldState, u32), _frame: &Frame) -> Scene {
    ...
    // R904 §5.36 — status bar: how many tokens the highlighter coloured,
    // derived from the same `style_runs()` the field paints (a highlighter is
    // attached, so `style_runs()` returns the syntax runs). Tagged so the demo
    // reads it as scene-as-data, the visible mirror of `scene/style_runs`.
    let text_state = use_text_edit_state(TF_TAG);
    let token_count = text_state.style_runs().len();
    let status = Scene::Container(
        ContainerNode::new(vec![Scene::Text(TextNode::styled(
            format!("{token_count} tokens highlighted"),
            Rect::default(),
            TextStyle::new()
                .with_size_px(STATUS_FONT_SIZE_PX)
                .with_fg(theme.resolve(ColorRole::OnSurfaceMuted)),
        ))])
        .with_tag(STATUS_TAG.to_owned()),
    );
...
impl WidgetCore for SyntaxView {
    ...
    fn create_external() -> Box<dyn External> {
        let text_state = use_text_edit_state(TF_TAG);
        // R904 §5.36 — seed a line of code and attach the syntax highlighter.
        // The font size is read from the field style so the derived runs paint
        // at the field's size (a run with the default size 0 is invisible).
    ...
    fn title() -> &'static str {
        "pinion hello-syntax-highlight (R904 §5.22 §5.36)"
    }
...
impl WidgetA11y for SyntaxView {
...
impl WidgetView for SyntaxView {
...
/// ([`SyntaxView::position_caret_for_point`]) and drag
/// ([`SyntaxView::select_drag_to_point`]) hooks — divergence between
...
mod tests {
    ...
    // ─────────────────────────────────────────────────────────────
    // R904 §5.36 — syntax highlighting binding
    // ─────────────────────────────────────────────────────────────
    ...
    fn r904_create_external_seeds_code_and_highlights() {
        ...
        with_owner(|| {
            ...
            let edit = use_text_edit_state(TF_TAG);
            assert_eq!(edit.text(), SEED_CODE, "seeded the code line");
            let runs = edit.style_runs();
            // "let n = 42 + \"x\" // sum" → keyword + number + string + comment.
            assert_eq!(runs.len(), 4, "four tokens coloured (kw/num/str/comment)");
            // The leading "let" is a keyword run at bytes 0..3.
            assert_eq!((runs[0].start, runs[0].end), (0, 3), "'let' keyword run");
        });
    }
    ...
    fn r904_typing_a_keyword_rehighlights() {
        ...
        with_owner(|| {
            let _ext = SyntaxView::create_external();
            let edit = use_text_edit_state(TF_TAG);
            edit.set_text(String::new()); // clear the seed
            assert!(edit.style_runs().is_empty(), "empty buffer, no tokens");
            edit.set_caret(0);
            edit.insert(KEYWORDS[0]); // type "let"
            assert_eq!(edit.style_runs().len(), 1, "typed keyword re-highlights");
        });
    }

    #[test]
    fn r904_view_carries_status_tag() {
        with_owner(|| {
            let scene = view((TextFieldState::Idle, 0), &Frame::default());
            assert!(
                scene.contains_tag(STATUS_TAG),
                "the syntax-status bar is present as scene-as-data",
            );
        });
    }
    ...
    fn r56_1_b_1_access_node_role_is_text_input() {
        with_owner(|| {
            let nodes = SyntaxView::access_node(&(TextFieldState::Idle, 0), None);
    ...
    fn r56_1_b_1_access_node_value_carries_live_text() {
        with_owner(|| {
            ...
            let nodes = SyntaxView::access_node(&(TextFieldState::Focused, 0), None);
            assert_eq!(
                nodes[0].value,
    ...
    fn r55_g20_view_contains_composite_paint_root_tag() {
        ...
        pinion_core::test_fixtures::assert_widget_view_carries_tag::<SyntaxView>(
            (TextFieldState::Idle, 0),
            &Frame::default(),
        );
    }
