//! pinion-forge codegen entrypoint for hello-window-focus.
//!
//! Compiles `app.pinion.xml` to `$OUT_DIR/app.rs`. `main.rs` pulls the
//! result in via `include!(concat!(env!("OUT_DIR"), "/app.rs"))`. Same
//! generic shape as every other example's build.rs.
