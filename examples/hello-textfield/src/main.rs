...
impl WidgetView for TextFieldView {
    ...
    fn ime_caret_rect(
        ...
        let (interaction, caret_byte) = *state;
        // ★ R1684.1 — the scene walk is the LIFTED helper's now, not this
        // binding's: `rect_for_tag` is a one-line wrapper over a method on
        // `Scene` itself, so the pinion-runtime dependency R657 was avoiding
        // was never needed to do the walk in pinion-widget-paint. Seven
        // bindings wrote these four lines.
        let theme = use_theme(THEME_TAG).theme_animated();
