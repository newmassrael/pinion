//! R933 §5.36 pinion-forge codegen entrypoint for hello-code-fold.
//!
//! Identical shape to hello-syntax-highlight's `build.rs`: compiles
//! `app.pinion.xml` to `$OUT_DIR/app.rs`, which `main.rs` pulls in via
//! `include!(concat!(env!("OUT_DIR"), "/app.rs"))`. On DSL-level failure
//! every diagnostic is printed on its own line before the build aborts.
