...
/// # Panics
///
/// If asked for a surface [`spec::SURFACES`] does not name, which is a defect
/// in this file.
#[must_use]
...
fn conformance_json() -> serde_json::Value {
    let surfaces: serde_json::Map<String, serde_json::Value> = spec::SURFACES
        .iter()
        .map(|&surface| {
            let canon = spec::canon(surface);
            let found = canon.diff(&built(surface));
            let ledger = spec::owed(surface);
            (
                surface.to_owned(),
                serde_json::json!({
                    "specified": canon.len(),
