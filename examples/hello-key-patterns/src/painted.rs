...
impl Painted {
    fn of(scene: &Scene, window: (u32, u32)) -> Self {
        ...
        for out in pinion_core::reach::out_of_sight(
            ...
            window,
            &mut stand_in_ink,
            // R2035 — this gate judges PLACEMENT with a stand-in measurer, so
            // the process's own faces are neither known nor consulted here, and
            // the arm that excuses nothing is the honest one.
...
fn r1730_the_regions_that_hold_the_surfaces_are_painted() {
    sweep(|_, shot, _, _, case| {
        // ★ R2063 — the heading row is named by the constant now rather than by
...
/// ★★★★★ R2141 — **what this screen publishes is pinned by its VALUE.**
///
/// The third consumer of `address_pin`. The rule, the fold, the message and the
/// regeneration live in the framework; this screen supplies only its own tag
/// set — including `reachable`, the marks a scroll would bring into view, which
/// a helper re-deriving from the scene would miss.
///
...
/// censuses, because paint, specification, wire, walks and tests all derive
/// from one constant and therefore all move together.
#[test]
fn r2141_every_published_address_is_pinned_by_value() {
