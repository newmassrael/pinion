...
【R684 plan】next round entry — **LayoutStyle::flex_basis substrate + dock/splitter layout-accuracy fixes + headless-RPC floating-window paint cycle**. Single round, 4 atomic on top of R683.C `761f09f`.

(0) **`pinion_core::style::LayoutStyle::flex_basis: Option<SizeValue>` field addition** — substrate-level extension to the §5.21 R23 layout primitive. Default `None` ≡ taffy's `flex_basis: auto` (current behaviour, every existing binding preserved bit-identical). New `LayoutStyle::with_flex_basis(SizeValue)` builder method. `pinion_runtime::layout::to_taffy_style` translation: `None → Dimension::Auto`, `Some(SizeValue::Px(n)) → length(n)`, `Some(SizeValue::Percent(p)) → percent(p/100)`. ~12 unit tests pin the round-trip + integration into the dock + splitter scenarios. honest LOC: ~+150 net.

(1) **`pinion_widget_paint::dock::view_dock_panel` header collapse fix** — replace `Size::px(0, header_height_px) + Auto`/`Stretch` collapse with explicit `LayoutStyle::new().flex(FlexDirection::Row).with_flex_basis(SizeValue::Px(0)).with_flex_grow(1.0).with_size(Size { width: Auto, height: Px(header_height_px) })`. The `flex_basis: Px(0)` + `flex_grow: 1.0` pair forces taffy to allocate full cross-axis to the header regardless of intrinsic content width (text title's 0-px intrinsic no longer collapses the strip). Header strip renders at full panel width (R683.C "1st consumer adoption" honest carry cleared). +1 substrate test pin (`r684_view_dock_panel_header_stretches_to_full_panel_width`). honest LOC: ~+40 net (mostly the test).

(2) **`pinion_widget_paint::splitter::view_splitter` flex_grow ratio fix** — replace left_child / right_child wrappers' `with_flex_grow(ratio)` with explicit `LayoutStyle::new().with_flex_basis(SizeValue::Px(0)).with_flex_grow(ratio).with_size(Size { width: Px(0), height: Auto })` so flex_basis is `0` rather than `auto`-derived from intrinsic. taffy then distributes the FULL parent extent proportionally to flex_grow (the R683.C "~50/50 regardless of declared ratio" honest carry cleared); pre-R684 splitter handle drag still mutated the ratio Signal correctly but the rendered split collapsed to ~50/50 under content-heavy panels. +2 substrate tests pin the ratio is honoured (`r684_splitter_left_width_matches_main_ratio_at_080`, `r684_splitter_left_width_matches_main_ratio_at_020`). honest LOC: ~+80 net.

(3) **Headless-RPC floating-window paint cycle** — investigate whether `pinion-shell::dispatch_rpc_for_window`'s `produce` closure should call `finalize_frame_for_window` as a side effect so `InputRouter::last_paint_scene` for dynamically-spawned windows is populated even when winit's `RedrawRequested` never fires (headless mode). Pre-R684 the AI-driven dock-back gesture goes through the direct `invoke("tear_off", Null)` channel (R683.C addition) because `scene/drag {window: "torn-..."}` would silently no-op on the empty InputRouter. R684 makes scene/drag work for floating windows under headless RPC. ~6-8 substrate tests + R683.C demo's dock-back sections converted back to drag-based (instead of invoke-based) for full UX coverage. honest LOC: ~+100 net.

(4) **44-demo regression sweep ×3 + commit + Mnemosyne consolidation + SEED handoff**. Commit `feat(widget-paint): R684 §5.16 §5.21 flex_basis + dock/splitter layout accuracy`. Mnemosyne `entry_id = R684` + impact_refs `[5.21, 5.16, 5.41]` + R683 carry (transform-aware fragment cache key, wgpu damage consumer, Phase B widget catalog expansion) preserved + R685 axis (likely DockSurface topology composition substrate — N-pane recursive split tree across `DockSlot::{Left, Right, Top, Bottom, Center}`).

honest LOC 예측: **R684 = +400-600 net** (per the substrate-extension Round estimate; smallest of the R683-class series — single primitive addition + 3 consumer call-site fixes + 1 dispatch-path trigger + demo regression preserve).

R684 후 가중 진척: ~30-32% → **~31-33%** (Phase B 25% × ~62% — dock 패널 헤더 + splitter ratio 실측 정확도 도달; AI-introspection-first dock-back gesture full drag UX). Phase C entry (game-loop / 3D scene graph / asset pipeline / physics / audio / PBR) 가 R&lt;700-900 range&gt;에서 자연 도래 — R684 이후 즉시 진입 가능.

**R684 verification mandatory** (라운드 끝):
- 44-demo regression sweep PASS deterministic 3 consecutive runs (R660-R683 + R684)
- Dock panel header rect width matches parent's cross-axis (≥ 100 px at 880-wide main / 360-wide floating)
- Splitter render width matches declared ratio (within 5% tolerance at 0.20 / 0.32 / 0.50 / 0.68 / 0.80)
- Headless-RPC scene/drag works on floating windows (R683.C demo section E converts back to drag-based)
- backward-compat — 단일 윈도우 binding 18개 + multi-window binding 회귀 0
- 부채 surface 정직 받아들임 — DockSurface topology composition (R685 carry), transform-aware fragment cache key carry (R682+1), wgpu damage consumer carry
