...
> **R686.C land (2026-05-28, commit `d03bd11`)** — `drive_hover_progress` → `use_hover_progress` hook lift. R686.B 에서 발견한 smell 청산: `drive_hover_progress(state) -> f32` (Owner::cache + `Animation<f32>` + `SpringConfig::default()` hover spring) 가 hello-button / figma-button-m3 / **hello-button-tui** 3곳 verbatim 복제 (cache-key const + panic string 만 차이 — Rule-of-Three 충족). **신규 substrate** `pinion_widget_paint::button::use_hover_progress(is_hover: bool, anim_key: &'static str) -> f32` — `ButtonState` 와 decoupled (bool predicate 받음 → 모든 hoverable widget 재사용 가능); +2 test (idle=0.0 + spring cached / per-key 독립). 3 binding 이 local fn 삭제 + `use_hover_progress(matches!(state, Hover), HOVER_ANIM_KEY)` 호출 (각 binding 의 unique `HOVER_ANIM_KEY` 유지 → 독립 spring). **hello-button-tui 가 pinion-widget-paint 의 첫 non-Vello consumer** (backend-agnostic crate, GPU/winit pull 없음 → TUI 안전). Import cascade 청산 (Animation/SpringConfig/Owner 를 3 binding non-test 에서 제거). **검증**: 66 test suites green (button 13 incl. 2 new + hello-button 13 + figma 18), clippy `-D pedantic` clean (float_cmp `assert_eq!(idle,0.0)` → `abs() < f32::EPSILON` 수정), **45/45 demo sweep PASS** (hello_button_r641 + figma_button_m3_r640 bit-identical; spring 거동 불변). Mnemosyne `R686.C` (ledger 559 → 560; T1 reject=0; impact_refs [5.16, 5.22, 5.28]). **정직 LOC: net ~-3** (substrate hook +75 incl. 2 test+doc; 3 verbatim ~14-line copy collapse) — 이번엔 진짜 LOC 감소 (R686.B 와 대조). **R686.A/B/C audit-clearance trilogy 종료** — button widget-catalog entry + hover hook 모두 SSOT, base clean. **다음 = R686 feature (drag-to-reorganize)**.
>
> **다음 세션 진입**: `load` 단독 입력. SEED 의 **R686 feature plan** 절 (DockSurface drag-to-reorganize handles — DockDropZone + topology mutation primitives + DockDragOverExternal + editor reorganize wire) 자동 진행. 모든 atomic 은 "비용 무관 + 장기 textbook canonical" 원칙.
>
> R686.C 가중 진척: Phase A 97% + Phase B 25% × ~78% + Phase C 35% × ~12% = 북극성 가중 **~34-36%**. audit-clearance trilogy 는 northern-star 수치 변화는 작지만 (SSOT 부채 청산), R686 feature (drag-to-reorganize) 가 Phase B 큰 상향 대기.

> **R686.B land (2026-05-28, commit `14fda5e`)** — view_button substrate lift (Smell 5). 핵심 발견: button **state machine** (`ButtonState`/`ButtonExternal`)은 이미 `pinion_core::widgets::button` 에 framework-owned; 중복된 건 **paint side** (M3 state-layer fill matrix + composition) 뿐이었음. R686.A 감사로 3 consumer (hello-button / figma-button-m3 / hello-dock-panels-editor) 가 fill matrix (0.08/0.12/0.38 계수 + `Color::lerp`)를 inline 복제 확인 (Rule-of-Three 충족). **신규 substrate** `crates/pinion-widget-paint/src/button.rs` (459 LOC): `ButtonColors` value-object (`filled_tonal`/`accent` theme ctor + `new` explicit-token ctor) + `ButtonStyle` geometry sidecar + `HOVER/PRESSED/DISABLED_STATE_LAYER` SSOT consts + `m3_button_fill(colors,state,hover_progress)` matrix + `view_button` composition; `ButtonState` 재사용. **핵심 설계**: consumer 간 모든 variance (colour source theme-role vs hardcoded-token / disabled fade-vs-switch / hover spring-vs-discrete / geometry) 를 **data 로 흡수** — Splitter/DockPanel sidecar 선례 그대로. 강제 추상화 회피: figma 는 `ButtonColors::new` 로 design token 직접 주입, editor 는 discrete `hover_progress = if Hover {1.0} else {0.0}` 전달 (spring 없이 bit-identical endpoint). **검증**: 66 test suites green (substrate 11 + hello-button 13 + figma 18 + editor 31), clippy `-D pedantic` clean, **45/45 demo sweep PASS** (hello_button_r641 + figma_button_m3_r640 + r685_editor_layout **bit-identical** post-migration). Mnemosyne `R686.B` (ledger 558 → 559; T1 reject=0; impact_refs [5.16]). **정직 LOC**: substrate +459, binding net -8 (148 ins/156 del) = net ~+451 — SSOT win (matrix 1곳), LOC 감소 아님 (R654 선례와 동일 정직 패턴). **clippy cascade 교훈**: fill 로직을 non-test 에서 빼면 helper/import 가 test-only 가 됨 → figma `button_fill_for`+`m3_button_fill` `#[cfg(test)]`, hello-button `Theme`/`Color` test-scope 이동; doc 줄머리 `+` 가 markdown bullet 로 오인돼 `doc_lazy_continuation` 발생 (→ "plus" 로 reword). **신규 발견 smell**: `drive_hover_progress` 가 hello-button + figma + **hello-button-tui** 3곳 verbatim 복제 (Owner::cache + Animation<f32> + SpringConfig) → R686.C `use_hover_progress` hook lift 후보.
>
> **다음 세션 진입**: `load` 단독 입력. SEED 의 **R686.C plan** 절 (drive_hover_progress → `use_hover_progress` reactive-hook lift) 또는 **R686 feature** (drag-to-reorganize) 자동 진행. 모든 atomic 은 "비용 무관 + 장기 textbook canonical" 원칙.
>
> R686.B 가중 진척: Phase A 97% + Phase B 25% × ~78% (widget catalog SSOT 1항목 — canonical M3 Button paint) + Phase C 35% × ~12% = 북극성 가중 **~34-36%**. R686 feature (drag-to-reorganize) 가 Phase B 큰 상향 대기.

> **R686.A land (2026-05-28, commit `756e4a3`)** — `load` 진입 후 사용자의 "hack/smell 없이 교과서적인가? SSOT 지켜지나? 비용 무관 장기 정통 구조인가?" 감사 요청에 대한 **정직한 후속**. R685.C SEED 의 "smell-free except S15" 주장은 **과장**이었음 — 실제 코드 grep+read 검증으로 확인된 gap 청산: **(1) stale `#[allow(dead_code)]` ×2** (`pinion-shell/src/app.rs`) — `primary_slot_mut` 은 호출처 0 (전 workspace grep 확인) → 진짜 dead code 라 **삭제**; `WindowSlot.spec_id` 는 `render_window` per-window redraw drain (app.rs:432) + `dispatch_rpc` window-scope 에서 실제 read 중 → **allow 만 제거** (field 유지). 두 suppression 의 reason "R670.B atomic 1 이 사용할 것" 은 15라운드째 거짓 — `view_for_window` dispatch 는 `substrate.rs` cached_state 경로로 배선됐고 (5개 call site + 2 binding override + 다수 test 로 fully wired 확인) 이 helper 들은 거치지 않았음. **(2) Smell 12** (splitter outer Surface fill) — `pinion_shell::paint_adapter::root_background` 가 root Container fill 을 Vello surface clear color 로 샘플링 (fallback `PenikoColor::BLACK`)하므로 이 fill 은 **load-bearing** (gutter/handle-gap backdrop + clear sample source; 제거 시 BLACK leak 재발 — redundant 아님). "remove vs document" carry 를 **keep + 주석 정정** 으로 종결. **(3) Smell 11** — `splitter::apply_flex_main` 의 no-`layout`-field wildcard (`Scene::Effect`/`Scene::Scroll` thin-Container auto-wrap; `Scene` 가 `#[non_exhaustive]` 라 미래 variant 도 이 arm 상속) 가 `r686_a_apply_flex_main_auto_wraps_layoutless_scenes` 로 pin (flex_basis(Px0)+flex_grow+1child+no-tag 계약). **검증**: cargo test --workspace green (baseline 3989 +1 = 3990; splitter suite 28/28), clippy `-D pedantic` clean (dead-code 삭제가 새 warning 0 → spec_id 실사용 + primary_slot_mut 진짜 dead 재확인), **45/45 demo sweep PASS** (release build; behavior-invariant 확인 — 삭제는 uncalled private fn, 나머지는 comment+test 뿐). Mnemosyne `R686.A` (atomic ledger 557 → 558; T1 reject=0; round-trip 1/1; GENERATED.md sync; impact_refs [5.16, 5.41]). **핵심 dock 토폴로지 substrate (private root + `try_new` validation gate + 통합 namespace validator `IdCollision` + SSOT walker — binding 이 tag/ratio drift 불가) 는 코드로 진짜 textbook + SSOT 임을 확인** — northern-star 방향 정렬 OK. **잔여 정직 carry**: Smell 5 (view_button substrate lift — R686.B), R686 feature (drag-to-reorganize), Smell 8 (Phase D persistence), S15 (Scene Clone architectural).
>
> **다음 세션 진입**: `load` 단독 입력. SEED 의 **R686.B plan** 절 (view_button substrate lift — Smell 5) 자동 진행. 모든 atomic 은 "비용 무관 + 장기 textbook canonical" 원칙.
>
> R686.A 가중 진척: 변화 없음 (behavior-invariant audit-clearance) — Phase A 97% + Phase B 25% × ~76% + Phase C 35% × ~12% = 북극성 가중 **~34-36%**. R686.B (view_button lift) → R686 (drag-to-reorganize) 진행 시 Phase B 상향.

> **R685.C land (2026-05-28, commit `aef13df`)** — 3차 textbook audit 후속, S9-S14 6건 인라인 청산. **S9** (apply_paint 2-primitive split): `ShellCore::apply_paint_for_window` → `apply_paint_for_window_with_hover_refresh` (winit paint loop용, refresh_hover synthetic arcs 발화) + `apply_paint_for_window_storage_only` (RPC dispatch용, set_paint_scene_for_window storage-only); dispatch_rpc_inner finalize hook이 inline 2-write 대신 명명된 storage_only primitive 사용. **S10** (compute_paint_scene_pure 통합): `compute_paint_scene_pure_internal(Option<&str>, w, h)` 도입, 기존 2 변형은 thin wrapper. **S11** (cross-namespace validation): `validate_node`가 panel_ids + split_ids 분리 HashSet → 단일 `HashMap<String, NodeKind>`; `TopologyError::IdCollision` variant 신규 (panel_id == split_id 시 InputRouter dispatch 충돌 차단). **S12** (Owner::cache Cow lift): `Owner::cache` / `cache_contains` key `&'static str` → `impl Into<Cow<'static, str>>`, CacheKey `(TypeId, &'static str)` → `(TypeId, Cow<'static, str>)`; R686 dock-reorganize 가 생성하는 runtime String ids 가 Box::leak 없이 cache slot addressing 가능 (Phase D editor blocker 청산); 정적 literal 은 Cow::Borrowed 무비용. **S13** (panic style): `DockTopology::single` 이 inline assert! → try_new().expect() 통과. **S14** (DRY): `DockTopology::for_each_split` substrate accessor; hello-dock-panels-editor binding-local for_each_split 헬퍼 제거. **Verification**: 3989 workspace tests PASS (+7 R685.C: 3 cross-namespace + 1 for_each_split + 3 Owner::cache Cow), clippy clean, 45-demo sweep PASS deterministic. **유일 잔여 architectural carry**: S15 (Scene Clone via External handle `Box<dyn>` → `Rc<RefCell<dyn>>` lift) — Scene Clone 가능 시 compute_paint_scene_pure 자체 불필요 (RPC dispatch가 producer 결과 직접 reuse); 모든 External impl ownership 모델 변경 broader refactor, R686+ Phase D entry 단계 자연 land (immediate-mode game-loop / dirty subtree cache 도 Scene Clone 유용). Mnemosyne entry `R685.C` (atomic ledger 556 → 557; T1 reject=0; impact_refs [5.16, 5.22, 5.35, 5.41, 5.49]). **R686 is the next round** — DockSurface drag-to-reorganize handles (Owner::cache Cow lift로 dynamic split id 준비 완료).
>
> **다음 세션 진입**: `load` 단독 입력. SEED 의 R686 plan 절 (drag-to-reorganize handles + topology mutation primitives) 자동 진행. 모든 atomic 은 "비용 무관 + 장기 textbook canonical" 원칙.
>
> R685.C 가중 진척: Phase A 97% + Phase B 25% × ~76% + Phase C 35% × ~12% (DockSurface substrate 가 SSOT + validation + Cow-key dynamic id + 명명 primitives 전부 textbook canonical; 3차 audit 통과로 hack/smell-free 도달 (S15 architectural carry만 잔여, 정직 명시); R686 drag-to-reorganize prerequisite 완성 — Owner::cache Cow lift 가 runtime id 지원, try_new validation gate 가 mutation primitive 안전성 보장) = 북극성 가중 **~34-36%**.

> **R684.B land (2026-05-28, commit `1b86f48`)** — R684 hack-inventory 4/5 청산 (Hack 0.1 + 3.1 + 3.3 + R685에서 이미 청산한 Hack 3.2 / 3.5 합산). **Substrate**: (a) `pinion_core::style::Size::auto()` + `with_width(SizeValue)` + `with_height(SizeValue)` composable builders (Hack 0.1) — narrow `width_px(u32)` / `height_px(u32)`는 ergonomic alias로 유지, `SizeValue` 전 variant (Auto/Px/Percent) 지원. (b) `pinion_shell::ShellCore::apply_paint_for_window(window_id, paint_scene, paint_layout)` substrate primitive (Hack 3.1) — `last_paint_layout` mirror + `InputRouter::update_paint_scene` 스토리지 쓰기 (refresh_hover synthetic arcs 포함, winit paint loop 용); `finalize_frame_for_window` 는 이제 `apply_paint_for_window + tail() + handle_tail()` composition으로 재설계. (c) `pinion_shell::ShellCore::compute_paint_scene_pure_for_window(window_id, w, h)` + `compute_paint_scene_pure(w, h)` (Hack 3.3) — V::view + compute_layout만; animation tick / immediate-mode tick / scroll-dirty re-run / animation-active redraw flag mutation 없음. RPC dispatch finalize hook 의 textbook 정통 primitive — producer closure가 이미 full pipeline을 1회 실행한 후 finalize에서 재실행 없이 pure variant로 결정적 geometry만 추출. (d) `dispatch_rpc_inner` post-finalize hook 재작성 — `compute_paint_scene_for_window` (full) → `compute_paint_scene_pure_for_window` (pure variant). 다중 step drag sequences 가 fresh hit-test geometry 보면서 side effect double-fire 없음 (todomvc_r660 회귀 영구 불가능 — Hack 3.2 substrate split + Hack 3.3 pure variant 가 합쳐 R684 atomic 3 first-paint gate workaround 의 양쪽 부작용 원천 차단). **Verification**: 3982 workspace tests PASS (+7 R684.B Size builder tests), clippy clean, 45-demo regression sweep PASS deterministic. **Honest 잔여**: Hack 3.4 (Scene Clone via External handle `Box<dyn>` → `Rc<RefCell<dyn>>` lift) — 모든 External impl ownership 모델 변경 broader refactor, R686+ Phase D entry 단계에서 자연 land (immediate-mode game-loop / dirty subtree cache 도 Scene Clone 유용). Mnemosyne entry `R684.B` (atomic ledger 555 → 556; T1 reject=0; impact_refs [5.16, 5.21, 5.35, 5.41]).
>
> **다음 세션 진입**: `load` 단독 입력. SEED 의 R686 plan 절 (DockSurface drag-to-reorganize handles + topology mutation primitives) 자동 진행.
>
> R684.B 가중 진척: Phase A 97% + Phase B 25% × ~75% + Phase C 35% × ~12% (R684 hack inventory 5/6 청산 — substrate 가 dispatch / paint / drain 모든 axis 에서 textbook canonical; R686 진입 prerequisite 완성; 부채 누적 영원 차단 — R684 atomic 3 first-paint gate 의 두 부작용 원천 모두 substrate split 으로 정통화) = 북극성 가중 **~34-36%**.

> **R685.B land (2026-05-28, commit `abc427f`)** — Post-R685 SSOT enforcement + DockTopology validation, after textbook audit surfaced 4 SSOT violations + redundancy carry. **Substrate refactor**: (a) `pinion_widget_paint::splitter::SplitterStyle::tag: &'static str` → `Cow<'static, str>` so dynamic split ids (R686 dock-reorganize) flow through `view_splitter` natively; `SplitterStyle` lost `Copy` (Cow::Owned not Copy), retains cheap `Clone`. (b) `view_dock_surface` walker SSOT rewrite — `panel_handle: Fn(&str) -> DockPanelHandle` + `split_handle: Fn(&str, SplitterOrientation) -> DockSplitHandle` → `panel_content: Fn(&str) -> Scene` + `split_state: Fn(&str, f32) -> DockSplitState`. Walker auto-builds `DockPanelStyle::m3_default(panel_id)` + `SplitterStyle::m3_default(orientation, split_id)` from the topology so the binding cannot drift the tags (S3+S4 SSOT enforcement). Walker passes the topology's declared `Split.ratio` as the `initial_ratio` arg to the binding's Signal constructor — no defaults duplication possible (S1 SSOT enforcement). `DockPanelHandle` struct removed, `DockSplitHandle` renamed to `DockSplitState` carrying only `ratio_signal` + `dragging`. (c) `DockTopology::try_new(root) -> Result<Self, TopologyError>` validation gate with `TopologyError::{DuplicatePanelId, DuplicateSplitId, InvalidRatio { split_id, ratio }, EmptyId}` enum + `Display` + `Error` impl; `DockTopology::new(root)` becomes thin `try_new(...).expect(...)` panic wrapper for hardcoded valid topologies. `DockTopology::root` pub field → `pub fn root(&self) -> &DockNode` accessor — future mutation primitives (R686 `swap_leaves` / `split_leaf_into` / `remove_leaf`) all return `Result<DockTopology, TopologyError>`, so invalid topology cannot exist (S2 SSOT enforcement). **Binding migration**: `hello-dock-panels-editor` `default_ratio_for_split` helper removed (S1 cleared — topology IS SoT); `split_tag_for` removed (R685 atomic 5d identity fn no longer needed); `split_cache_key_for` retained for `Owner::cache &'static str` lifetime requirement; new `for_each_split` helper walks topology Split nodes for `create_extra_externals` registration. View fn simplified to `view_dock_surface(&topology, panel_content_callback, split_state_callback, &theme)` with 4-line callbacks each. **Verification**: 3975 workspace tests PASS (R685 baseline 3966 → +9 R685.B topology validation tests covering duplicate panel_id / duplicate split_id / NaN ratio / out-of-range ratio / empty panel_id / empty split_id / valid editor / boundary ratios 0.0 + 1.0 / root accessor). cargo clippy clean under workspace `-D pedantic`. 45-demo regression sweep PASS deterministic (R660-R685.B; all dock binding behavior bit-identical). Mnemosyne entry `R685.B` (atomic ledger 554 → 555; T1 reject=0; impact_refs [5.16, 5.41, 5.49]). **Honest carry**: R684.B remaining 4 hacks (Hack 0.1 Size builder / 3.1 apply_paint split / 3.3 compute_paint_scene_pure / 3.4 Scene Clone via External Rc lift — broader refactor R686+), Smell 5 (view_button substrate lift; 5+ inline consumers Rule-of-Three audit), Smell 8 (Topology + Signal persistence contract for Phase D editor workspace save/restore — R686 dock-reorganize land 시 함께 설계), Smell 11 (apply_flex_main Scroll/Effect wildcard untested — Scroll-in-splitter use case 등장 시 구체 테스트), Smell 12 (R683.C splitter Surface fill backstop redundancy post-R685 — remove vs document). **R686 is the next round** — DockSurface drag-to-reorganize handles (DockDropZone + edge-detection + mutation primitives swap_leaves / split_leaf_into / remove_leaf, all validated through try_new path).
>
> **다음 세션 진입**: `load` 단독 입력. SEED 의 R686 plan 절 (drag-to-reorganize handles + topology mutation primitives) 자동 진행. 모든 atomic 은 "비용 무관 + 장기 textbook canonical" 원칙.
>
> R685.B 가중 진척: Phase A 97% + Phase B 25% × ~72% + Phase C 35% × ~10% (DockSurface substrate SSOT 정통 enforced — topology IS source of truth for panel ids, split ids, orientations, initial ratios; validation gate prevents invalid topology reaching walker / mutation primitives; binding consumer code 25% 더 짧음 (helpers + duplication 제거); Phase D editor self-hosted prerequisite chain 의 next-step prerequisite (drag-to-reorganize) 가 R686 에서 자연 land) = 북극성 가중 **~33-35%**.

> **R685 land (2026-05-28, commit `76d5b8b`)** — DockSurface topology composition substrate + 26th example binding `hello-dock-panels-editor` (2nd dock consumer) + Smell 1+2+3+4+6 inline textbook 청산 + R684.B Hack 3.2 substrate split. **Substrate land**: (a) `pinion_widget_paint::dock::{DockTopology, DockNode, view_dock_surface, DockPanelHandle, DockSplitHandle, view_floating_placeholder, FloatingPlaceholderStyle, floating_window_id, DEFAULT_FLOATING_WINDOW_PREFIX}` — recursive binary split tree (`DockNode::Split { id, orientation, ratio, first, second }` + `DockNode::Leaf { panel_id }`) with serde derive + DF pre-order `panel_ids()` / `split_ids()` traversal helpers + `DockTopology::single` / `new` constructors. (b) `view_dock_surface(topology, panel_handle, split_handle, theme) -> Scene` recursive walker — `panel_handle: Fn(&str) -> DockPanelHandle` looks up Leaf state by stable `panel_id`; `split_handle: Fn(&str, SplitterOrientation) -> DockSplitHandle` looks up Split state by stable `id` (NOT positional index — Phase D editor topology mutation safety). **Smell audit clearance (6/6 inline + 1 R684.B carry partial)**: Smell 1 (splitter wrapper layout fill) — `view_splitter` apply_flex_main eliminates the redundant wrapper Container, places `flex_basis(Px(0)) + flex_grow(ratio)` directly on each child Scene's LayoutStyle. Pre-R685 wrapper's Display::Block + Size::Auto made taffy fall back to intrinsic sizing for the wrapper's inner content (the R685 5-pane editor occupied only the top ~50% of the viewport, and hello-dock-panels panels under-filled too; post-fix both bindings fill viewport correctly, layout probe shows inner_v 748px / middle_h 580px / console 164px against 800-px viewport). Smell 2 (split-index positional addressing) — `DockNode::Split.id: Cow<'static, str>` stable field replaces walker's depth-first counter; topology mutation (leaf insert / Split rebalance / future dock-reorganize) keeps every binding's per-Split state bound to the right Split. Smell 3 (DockSlot dead data) — `DockSlot` enum + `Leaf::slot` field + `slot_for_panel` helpers all removed; no consumer used them, [[abstraction-needs-second-consumer]] re-add on real 2nd consumer signal. Smell 4 (4-fn `use_split_X_ratio` duplication in editor binding) — single `use_split_ratio(id, default)` helper; `create_extra_externals` iterates `topology.split_ids()` for single source of truth. Smell 6 (defensive `_ =>` fallback arms) — `split_tag_for` / `default_ratio_for_split` now `unreachable!()` contract panic on unknown ids. Smell 5 (`view_button` substrate lift; 5+ inline consumers across hello-button / figma-button-m3 / todomvc / settings-panel / hello-dock-panels / hello-dock-panels-editor) deferred to R686+ as a dedicated Rule-of-Three audit round. Splitter handle cross-axis fix: `Size::px(handle_extent_px, 0)` → `Size::width_px(...)` / `Size::height_px(...)` (R684 substrate) so AlignItems::Stretch promotes handle rect to full cross-axis extent (pre-R685 explicit Px(0) pinned the handle to literal 0 height, hit-test unreachable; layout probe shows 4×600 post-fix). **R684.B Hack 3.2 substrate split** (1 of 6 hack inventory items cleared): `pinion_runtime::InputRouter::{set_paint_scene, refresh_hover_for_all_active_pointers}` split out of the composed `update_paint_scene`; `pinion_runtime::CoreShell::set_paint_scene_for_window` + `pinion_shell::ShellCore::dispatch_rpc_inner` finalize hook calls `set_paint_scene` unconditionally per RPC (no longer first-paint-gated). Multi-step drag sequences in headless RPC see fresh hit-test geometry per RPC dispatch without firing synthetic `PointerEnter` / `PointerLeave` arcs that double-mutate widget state (the R660 RadioGroup regression bisect that triggered R684's first-paint-only workaround is now correctly addressed via primitive split). **2nd consumer Rule-of-Three lift evidence**: `view_floating_placeholder` substrate lift via `hello-dock-panels` (R683.C 1st consumer) → R685 `hello-dock-panels-editor` (2nd consumer). 1st consumer's binding-local `view_floating_placeholder` becomes a 4-line wrapper `view_floating_placeholder_for` that calls the substrate version with the binding-local font size override; bit-identical visual rendering preserved (R683 demo PASS 7.46s post-retrofit, 29 binding tests PASS). **R685 editor binding** (~1100 LOC): 5-pane DCC/IDE editor layout (toolbar + outliner + viewport + properties + console) via declarative `DockTopology` + `view_dock_surface`; viewport hosts an interactive Button (only SCXML-bearing widget); 4 `SplitterExternal`s registered per `topology.split_ids()` walk; PINION_SCREENSHOT visual confirms 1200×800 viewport fully populated by the 5-pane topology. **Demos**: `tools/demos/r685_editor_layout.py` (8 sections A-H, ≥40 assertions: substrate sanity / 5-pane topology / splitter slots / orientations / button click / inner-H splitter drag via raw coords / topology determinism / panel content sanity), 1.74s. `tools/demos/r683_dock_tear_off.py` section C splitter drag converted to raw coords via `_splitter_handle_x` helper (queries `scene/layout` for the splitter's `children[1]` handle rect — composite-tag dispatch incompatible with SplitterExternal's splitter-wide cursor-fraction semantics); section E dock-back converted to drag-based via R684 atomic 3 first-paint finalize + R685 Hack 3.2 split. **R685 verification**: 45-demo regression sweep PASS deterministic (R660-R684 + R685; r685_editor_layout 1.74s, r683_dock_tear_off 7.46s, todomvc_r660 6.68s, r679_devtools_bidirectional_select 4.59s, r680_per_window_owner_scope 2.39s; full 45-demo sweep ran 0 failures). cargo test --workspace PASS (3966 tests, 0 fail; R684 baseline 3910 → +56 R685 incl. 13 topology + 7 surface + 5 placeholder + 31 editor binding + 6 splitter handle/wrapper + InputRouter set/refresh split tests). cargo clippy --workspace --all-targets --features pinion-runtime/vello clean under workspace `-D pedantic`. Mnemosyne entry `R685` (atomic ledger 553 → 554; T1 orphan total 0; round-trip 1/1 + GENERATED.md sync; impact_refs [5.16, 5.21, 5.35, 5.41, 5.49]). **R686 is the next round** — DockSurface drag-to-reorganize handles (Phase B/D editor entry: drop targets on adjacent panel edges to swap / reparent within the topology). Honest carry: R684.B remaining 5 hacks (Hack 0.1 Size::with_width/height SizeValue builder / Hack 3.1 apply_paint_for_window split / Hack 3.3 compute_paint_scene_pure / Hack 3.4 Scene Clone via External Rc lift / Hack 3.5 dispatch_rpc_inner textbook rewrite), view_button substrate lift (Smell 5; 5+ inline consumers Rule-of-Three audit).
>
> **다음 세션 진입**: `load` 단독 입력. SEED 의 R686 plan 절 (DockSurface drag-to-reorganize handles — adjacent-panel-edge drop targets for swap/reparent gestures; topology mutation reactive substrate; 1st mutation-aware consumer) 자동 진행. 모든 atomic 은 "비용 무관 + 장기 textbook canonical" 원칙. 매 atomic 종료 시 cargo test + clippy + 45-demo regression sweep 검증.
>
> R685 가중 진척: Phase A 97% + Phase B 25% × ~70% + Phase C 35% × ~10% (DockSurface topology composition substrate + 2nd dock consumer + Smell 1-4+6 inline 청산 + R684.B Hack 3.2 substrate split — Phase D editor self-hosted 진정 first concrete step; topology-as-data + serde derive + stable Split id addressing 가 editor workspace persistence + drag-to-reorganize 의 textbook prerequisite; 5-pane editor visual evidence 가 northern-star canvas 첫 visible approximation) = 북극성 가중 **~33-35%**. R686 (drag-to-reorganize handles) 완료 시 ~35-38% (Phase B ~80%; topology mutation gesture가 editor 정통 진입).

【R686.C plan (DONE — commit `d03bd11`)】 `drive_hover_progress` → `use_hover_progress` reactive-hook lift. R686.B 에서 발견: `drive_hover_progress(state) -> f32` (Owner::cache + `Animation<f32>` + `SpringConfig::default()` 로 Idle↔Hover spring progress 구동) 가 `hello-button` / `figma-button-m3` / `hello-button-tui` 3곳 **verbatim** 복제 (Rule-of-Three 충족). `pinion_widget_paint::button::use_hover_progress(is_hover: bool, anim_key: &'static str) -> f32` 으로 lift — Owner::current() resolve + cache + set_target + value. 3 binding 이 `let hp = use_hover_progress(matches!(state, Hover), KEY);` 로 축약. editor 는 discrete 라 미사용 (그대로). ~2-3 atomic. 검증: 3 binding bit-identical (spring settle 테스트 포함) + 45-demo sweep. impact_refs [5.16, 5.22, 5.28]. 그 후 **R686 feature** (drag-to-reorganize) 가 Phase B 큰 상향.

【R686.B plan (DONE — commit `14fda5e`)】 view_button substrate lift (Smell 5). R686.A 감사로 실제 중복 확인 (≥3 consumer, Rule-of-Three 충족): `hello-button::button_fill_endpoints` (Idle/Hover endpoint lerp) + `figma-button-m3::button_fill_for` (+ `BTN_RADIUS` corner) + `hello-dock-panels-editor::view_viewport_button`. 모두 "M3 state→fill state-layer lerp" 정통 로직을 inline 복제 — `DockPanelStyle::m3_default` / `SplitterStyle::m3_default` 와 동일 패턴의 substrate 부재. 단 settings-panel 은 button 이 아니라 toggle/slider/nav-row 이므로 consumer 아님 (SEED 의 "5-6 consumers" 는 약간 과장; 진짜 3 consumer). R686.A 감사로 실제 중복 확인 (≥3 consumer, Rule-of-Three 충족): `hello-button::button_fill_endpoints` (Idle/Hover endpoint lerp) + `figma-button-m3::button_fill_for` (+ `BTN_RADIUS` corner) + `hello-dock-panels-editor::view_viewport_button`. 모두 "M3 state→fill state-layer lerp" 정통 로직을 inline 복제 — `DockPanelStyle::m3_default` / `SplitterStyle::m3_default` 와 동일 패턴의 substrate 부재. 단 settings-panel 은 button 이 아니라 toggle/slider/nav-row 이므로 consumer 아님 (SEED 의 "5-6 consumers" 는 약간 과장; 진짜 3 consumer).

(0) **`pinion_widget_paint::button` 신규 module** — `ButtonStyle` sidecar (`#[non_exhaustive]`; corner_radius / label_font_size_px / role(ColorRole) / padding) + `ButtonStyle::m3_default()` + `button_fill_for_state(theme, style, state) -> Color` (Idle/Hover/Pressed M3 state-layer lerp, linear-space per [[color-lerp-linear-space]]) + `view_button(label, state, theme, style) -> Scene`. `ButtonState` enum 은 기존 binding 들이 각자 정의 — substrate 가 canonical `{Idle, Hover, Pressed}` 제공할지, 아니면 fill helper 만 제공하고 state 는 binding 소유로 둘지 첫 atomic 에서 결정 (SCXML widget state 와의 관계 고려). ~+200 LOC + ~10 tests.

(1-3) **3 consumer migration** — hello-button / figma-button-m3 / hello-dock-panels-editor 각각 inline fill fn → substrate 호출. figma 의 corner radius 는 `ButtonStyle::with_corner_radius`. 각 binding 의 paint test 가 bit-identical 유지 (screenshot/RPC 회귀 0) 확인.

(4-5) **demo/verify + 45-demo sweep + commit + Mnemosyne `R686.B`**. 기존 figma_button_m3_r640 / hello_button_r641 demo 재사용 (substrate migration 후 bit-identical). honest LOC: substrate +200, 3 binding 각 -20~40 → **net ~0 ~ +120** (R654 view_button-미포함 cascade 의 per-binding ~25 LOC 절감 패턴 참조). impact_refs [5.16].

R686.B 후: Phase B 25% × ~76% → ~78% (widget catalog SSOT 1 항목 추가; northern-star 변화 미미하나 SSOT 부채 청산). 그 다음 R686 feature (drag-to-reorganize) 가 Phase B 큰 상향.

【R686 plan (feature, R686.B 이후)】 — **DockSurface drag-to-reorganize handles**. Phase B/D editor 정통 진입의 다음 step — adjacent panel edge 위 drop targets 가 swap / reparent / split-and-insert gestures 를 지원하면 user 가 editor layout 을 mouse 만으로 자유롭게 재배치 가능. 5-pane editor 가 "사용자가 직접 customise 한 layout" 으로 전환되는 첫 라운드. 5-6 atomic.

(0) **`pinion_widget_paint::dock::DockDropZone` enum + edge-detection** — `#[non_exhaustive]` enum `DockDropZone::{None, SwapLeft, SwapRight, SwapTop, SwapBottom, Replace}` 매핑 + `dock_drop_zone_for(panel_rect, cursor_at) -> DockDropZone` pure helper (cursor 가 panel rect 의 edge 근접 영역에 있으면 해당 edge의 swap zone; 중앙은 Replace; 외부는 None). ~+150 LOC + ~8 tests.

(1) **`DockTopology` mutation primitives** — `swap_leaves(panel_id_a, panel_id_b) -> Option<DockTopology>` (depth-first search + Box swap; topology shape unchanged) + `split_leaf_into(panel_id, new_leaf_panel_id, new_split_id, new_orientation, new_ratio, position: First|Second) -> Option<DockTopology>` (Leaf 를 Split 으로 lift + 기존 leaf 를 new position 으로 + 새 leaf 추가) + `remove_leaf(panel_id) -> Option<DockTopology>` (Leaf 제거 + 부모 Split 도 제거하고 sibling promote). 모든 mutation 은 `&self -> Option<Self>` immutable form (None = panel_id not found 또는 root removal). ~+200 LOC + ~12 tests.

(2) **`DockDragOverExternal` substrate** — drag-over state machine: hover panel + edge → emit `DockReorganizeIntent::{Swap | SplitInsert | Replace}` intent on PointerUp. 새 External, `wants_pointer_capture=false` (drag-over는 hover-tracking; capture는 source 의 DockPanelExternal 에서). drag source 와 drop target identification via shared signal handle. ~+250 LOC + ~10 tests.

(3) **`hello-dock-panels-editor` reorganize wire** — reducer arm for `DockReorganizeIntent`; `Signal<DockTopology>` Owner::cache 등록; mutation 후 `Signal::set(new_topology)` → reactive view fn re-run → splitter signals 재등록 (Owner::cache 가 stable id 로 dedup 해야 함; `use_split_ratio(id, default)` 가 이미 stable). ~+150 LOC + ~10 tests.

(4) **`tools/demos/r686_dock_reorganize.py`** — drag panel header onto adjacent panel edge → swap (substrate test for `swap_leaves`); drag onto center → replace; drag onto panel-empty corner → new SplitInsert. ≥40 assertions, R685 5-pane editor 위에서 검증.

(5) **45-demo regression sweep + commit + Mnemosyne `R686`**. `R685` baseline 회귀 0. impact_refs [5.16, 5.45, 5.49]. honest LOC 예측: **R686 = +1200-1700 net**.

R686 후 가중 진척: ~33-35% → **~35-38%** (Phase B ~80% — drag-to-reorganize 가 editor 의 "customize 가능한 layout" 정통 진입; Phase D editor self-hosted 가 visible 한계점에 도달). Phase C entry (game-loop / 3D / assets) 가 R<700-800 range>에서 자연 도래.

**R685 verification (라운드 끝, completed)**:
- ✓ 45-demo regression sweep PASS (R660-R684 + R685; deterministic 1 sweep — `1 sweep PASS` 단순 충분 검증, "3 consecutive runs" 종전 SEED 의 reflex 보다 textbook 정통)
- ✓ cargo test --workspace PASS (3966 tests; R684 baseline 3910 → +56 net)
- ✓ cargo clippy --workspace --all-targets --features pinion-runtime/vello clean under workspace `-D pedantic`
- ✓ Smell audit 6/6 inline 청산 (Smell 1 splitter fill, Smell 2 stable Split id, Smell 3 DockSlot 제거, Smell 4 use_split_X_ratio dedup, Smell 6 unreachable!() contract panics) + 1 R684.B carry partial (Hack 3.2 substrate split)
- ✓ R685 editor PINION_SCREENSHOT visual 검증 — 1200×800 viewport 가 5-pane topology 로 완전 채워짐
- ✓ hello-dock-panels visual + RPC 행동 bit-identical post-substrate refactor (29 binding tests PASS + r683_dock_tear_off.py PASS 7.46s)
- ✓ Mnemosyne entry R685 clean validate (T1 reject=0, GENERATED.md sync, atomic ledger entries 553 → 554, orphan_refs 5+0 frozen)
- ✓ 부채 surface 정직 받아들임 — R686 drag-to-reorganize handles (다음 라운드) / R684.B 잔여 5 hacks (Hack 0.1 / 3.1 / 3.3 / 3.4 / 3.5; 별도 R685.B 또는 R686+ carry) / Smell 5 view_button substrate lift (5+ inline consumers Rule-of-Three audit) / Smell 7 (splitter outer Container Surface fill 이 R683.C atomic 4 backstop 으로 less critical 됐지만 유지)

> **R684 land (2026-05-28, commit `14bf51b`)** — `pinion_core::style::LayoutStyle::flex_basis` substrate + dock-header cross-axis stretch fix + splitter ratio proportional split + headless-RPC floating-window first-paint finalize. **Atomic 0** — new `pub flex_basis: Option<SizeValue>` field on `LayoutStyle` (default `None` ≡ taffy `Dimension::Auto`, pre-R684 layout graph bit-identical for every existing binding) + `with_flex_basis(SizeValue)` builder + manual `Hash` impl extension (paint-fragment cache key now includes the new field). New `Size::height_px(h)` / `Size::width_px(w)` cross-axis-pinning constructors land alongside because `Size` is `#[non_exhaustive]` and cross-crate code cannot use struct expressions. `pinion_runtime::layout::to_taffy_style` lowers via `if let Some(basis) = layout.flex_basis { s.flex_basis = to_dimension(basis); }` — the `None` branch leaves taffy's struct default (`Dimension::Auto`) so no observable change. 7 substrate tests in `pinion-core::style::tests` (field default / builders / PartialEq / Hash) + 5 in `pinion-runtime::layout::tests` (`None` preserves intrinsic / `Px(0)+flex_grow(1.0)` claims full parent / `Percent(50)` resolves to half / two-sibling 0.3/0.7 split proportionally / `Some(Auto)` ≡ `None`). **Atomic 1** — `pinion_widget_paint::dock::view_dock_panel` header strip layout: `Size::px(0, header_height_px)` → `Size::height_px(style.header_height_px)` so cross-axis stays `Auto` and the outer Column container's `AlignItems::Stretch` can promote to full panel width (R683.C "first-consumer adoption" honest carry cleared; the strip no longer collapses to padding-only 16px). Note: dock header does NOT use `flex_basis(Px(0))+flex_grow(1.0)` — that would compete with the content_wrapper's `flex_grow(1.0)` for the Column-parent Y-axis and break the fixed-height header invariant. The splitter (atomic 2) is the canonical `flex_basis` consumer; dock header is the cross-axis-stretch fix only. Pinned by `r684_view_dock_panel_header_stretches_to_full_panel_width` (real `compute_layout` pass over a 400×300 viewport asserting `header.rect.w == panel_w` AND `header.rect.h == header_height_px`). **Atomic 2** — `pinion_widget_paint::splitter::view_splitter` left/right wrappers: `.with_flex_grow(ratio)` → `.with_flex_basis(SizeValue::Px(0)).with_flex_grow(ratio)` so taffy distributes the FULL parent extent proportionally to `flex_grow` rather than the leftover after intrinsic-content basis resolution. Pre-R684 the rendered split collapsed toward ~50/50 under content-heavy panels (the canonical "flex-basis: 0; flex-grow: r" CSS idiom every flex panel system ships); pinned by 2 substrate tests at ratios 0.80 and 0.20 against a 1000-px parent (each asserts wrapper width within 5% of declared ratio AND `L + handle + R == parent_w` invariant). `pinion-widget-paint` Cargo.toml gains `pinion-runtime` as a test-only dev-dependency so the dock + splitter substrate tests can drive real `compute_layout` passes (no production code impact; vello feature stays gated). **Atomic 3** — `pinion-shell::ShellCore::dispatch_rpc_inner` gains a post-dispatch headless-RPC finalize hook: when `window_id.is_some()` AND the produce closure ran AND the addressed window's `InputRouter::has_last_paint_scene()` is `false` (truly never-painted floating window), re-runs `compute_paint_scene_for_window` + writes the result into `InputRouter::last_paint_scene` + the `ShellCore.last_paint_layout` substrate mirror. **First-paint gate is load-bearing**: `InputRouter::update_paint_scene` synthesises `PointerEnter` via `refresh_hover` for every active cursor as a side effect (legitimate for window resize / re-layout cases where widget rects move under a stationary cursor). Re-driving it on every RPC dispatch double-fires hover transitions and mutates widget state — the bisect signal that caught this: `tools/demos/todomvc_r660.py` End-key assertion regressed (active filter expected 2, observed 1) because the post-finalize `refresh_hover` dispatched a `PointerEnter` that the RadioGroup statechart interpreted as a focus shift. The first-paint gate scopes the fix to truly-fresh floating windows where this side effect is desired (no winit paint has run yet, no cursor state to clobber). Bypasses `finalize_frame_for_window` because its trailing `tail()` + `handle_tail()` would double-drain pending intents (the dispatch's existing R51.123 post-dispatch `handle_tail` is the canonical drain site). New public accessors land for the gate + tests: `InputRouter::has_last_paint_scene()`, `CoreShell::has_last_paint_scene_for_window()`, `ShellCore::has_last_paint_scene_for_window()` + `ShellCore::last_paint_layout()`. 7 substrate tests in `pinion-shell::tests::dispatch_core::r684_headless_rpc_floating_window_finalize` (first paint-touching RPC populates router / single-window `dispatch_rpc` with `None` does NOT finalize / read-only `focus/get` skips finalize / sibling isolation / repeat dispatches stay populated / `last_paint_layout` mirror updates / unknown window id finalizes lazily). **R684 verification**: 44-demo regression sweep PASS deterministic ×3 consecutive runs (R660-R683.C + R684 substrate additions; per-sweep wall time ~150s; all demos green every run; todomvc_r660 regression bisected end-to-end then fixed). cargo test --workspace PASS (3910 tests / 0 fail; R683.C 3844 baseline → R684 3910). cargo clippy --workspace --all-targets --features pinion-runtime/vello clean. Mnemosyne entry `R684` (atomic ledger 552 → 553; T1 orphan total 0; round-trip 1/1 + GENERATED.md sync; impact_refs [5.21, 5.16, 5.41]). **R685 is the next round** — DockSurface topology composition substrate (N-pane recursive split tree across `DockSlot::{Left, Right, Top, Bottom, Center}`) per the seed [`abstraction-needs-second-consumer`] 2nd-consumer plan.
>
> **다음 세션 진입**: `load` 단독 입력. SEED 의 R685 plan 절 (DockSurface topology composition substrate — N-pane recursive split tree + 2nd dock consumer binding to trigger the substrate lift per Rule-of-Three) 자동 진행. 모든 atomic 은 "비용 무관 + 장기 textbook canonical" 원칙. 매 atomic 종료 시 cargo test + clippy + 44-demo regression sweep 검증.
>
> R684 가중 진척: Phase A 97% + Phase B 25% × ~62% + Phase C 35% × ~8% (LayoutStyle::flex_basis substrate land — CSS-canonical flex distribution primitive on the §5.21 R23 layout type; splitter ratio + dock header layout-accuracy fixes via the same substrate; headless-RPC floating-window first-paint finalize hook + first-paint gate textbook-canonical scoped to avoid `refresh_hover` side-effects on already-active windows; bisect-driven root cause discovery + textbook fix preserved every binding's keyboard/pointer interaction semantics) = 북극성 가중 **~31-33%**. R685 (DockSurface topology composition + 2nd dock consumer) 완료 시 ~33-35% (Phase B ~70% — recursive dock topology + drag-to-reorganize substrate; Phase D editor self-hosted natural land starts here).

> **R683.C land (2026-05-27, commit `761f09f`)** — 4-axis paint-pipeline rewrite series Round 4 of 4 close (atomics 4+5 of 5). R683 land complete across 3 sessions (`c0b853d` R683.A window-lifecycle substrate + `aaf854f` R683.B Splitter+DockPanel widget substrate + `761f09f` R683.C 1st consumer + cascade closure + 3 R683.B inline substrate fixes). **R683.C atomic 4** — `pinion_widget_paint::devtools::ClickRouter` substrate lift via Rule-of-Three (`hello-multi-window` R679 = 1st consumer, `hello-dock-panels` R683.C = 2nd consumer): `pub struct ClickRouter { last_clicked, pending }` + `impl External` (Backend::Gui+Tui+Rpc + Repaint::Framework + UiThreadSync + drain_intents + is_dirty) + `impl ExternalIntrospect` (schema slots `last_clicked` + `click`, query/intervene/invoke); 14 substrate regression tests pin the wire shape; `hello-multi-window` MainWindowClickRouter struct + 3 impl blocks removed (~110 LOC -) and `ClickRouter::new()` consumed via `use pinion_widget_paint::devtools::ClickRouter`; binding-side tag constants + reducer arm dotted-form intent (`main_click_router.click`) preserved bit-identical. **R683.C atomic 5** — `examples/hello-dock-panels` 25th example binding (~1180 LOC): 3 dock panels (inspector tree mirror + property pane + viewport `Button`) composed via `Splitter(H)` wrapping `Splitter(V)`; opt-in `WidgetView::windows_signal -> Some(Rc<Signal<Vec<WindowSpec>>>)` (R683.A first reactive-topology binding); `dock_or_placeholder` substitution per panel keyed by `use_windows_topology` Signal (`is_panel_floating` predicate); floating-window id prefix `torn-{panel_id}` with `Cow::Owned(format!(...))`; reducer arms for `{inspector,property,viewport}.tear_off` (toggle floating) + `inspector_tree.click/hover` (cross-panel select arc) + `viewport_click_router.click` (R679 cascade closure) + `viewport_btn.click` (user-mouse half); 29 binding tests + tools/demos/r683_dock_tear_off.py (40+ assertions across A-J sections — drag-based tear-off / direct invoke-based dock-back / multi-panel cascade / cross-panel selection / property pane updates). **R683.C inline substrate fixes** (3 R683.B gaps surfaced by 1st consumer adoption): (i) `pinion_widget_paint::splitter::view_splitter` outer Container fills with `theme.resolve(ColorRole::Surface)` to override `VelloContext` default `PenikoColor::BLACK` clear — pre-R683.C dock + tear-off bindings rendered against BLACK leak-through under un-filled splitter strips; pinned by `r683_c_view_splitter_outer_container_filled_with_theme_surface`. (ii) `pinion_widget_paint::dock::DockPanelExternal` `ExtraExternal` registration moved from composite tag `{panel_tag}#header` to panel root tag `{panel_tag}` — the R51.42 `dispatch_send`/`forward_pointer_move` split-on-`#` discipline forces externals onto the primary half of composite paint tags, so R683.B's first-cut registration made the InputRouter silently skip every press (no `pointer_move` ever reached the External). The fix adds an `is_drag_armed: Cell<bool>` gate driven by the `"header:PointerDown"` vs `"content:PointerDown"` sub-index prefix the R51.42 wire payload carries so content-area presses cannot accidentally drive tear-off through the same External. (iii) `DockPanelExternal` direct `invoke("tear_off", Null)` channel added — enqueues the same `tear_off` intent the drag path would, resets drag bookkeeping to fresh post-`PointerUp` state. Bypasses the drag simulation for headless RPC dock-back testing (winit may never fire `RedrawRequested` for floating windows under headless mode, leaving their per-window `InputRouter::last_paint_scene` empty; the direct invoke path lets AI clients drive the dock-back gesture without depending on winit's paint timing). Schema slot `tear_off` advertised. **R683.C verification**: 44-demo regression sweep PASS deterministic ×3 consecutive runs (R660-R682 + R683.C; r683_dock_tear_off.py 7.30s; per-sweep total ~150s). cargo test --workspace PASS (65 suites, 0 failures). cargo clippy --workspace --all-targets --features pinion-runtime/vello clean. Mnemosyne entry `R683.C` (atomic ledger entries 551 → 552; T1 orphan total 0; round-trip 1/1 + GENERATED.md sync; impact_refs [5.16, 5.41, 5.45, 5.49]). **R684 is the next round** — dock panel header collapse fix (LayoutStyle::flex_basis substrate addition to pinion-core::style) + splitter flex_grow ratio fix (same axis) + headless-RPC floating-window paint cycle.
>
> **다음 세션 진입**: `load` 단독 입력. SEED 의 R684 plan 절 (`pinion_core::style::LayoutStyle::flex_basis` field 추가 + dock panel header collapse + splitter flex_grow ratio + headless-RPC floating-window finalize_frame 트리거) 자동 진행. 모든 atomic 은 "비용 무관 + 장기 textbook canonical" 원칙. 매 atomic 종료 시 cargo test + clippy + 44-demo regression sweep 검증.
>
> R683.C 가중 진척: Phase A 97% + Phase B 25% × ~58% + Phase C 35% × ~8% (4-axis paint-pipeline rewrite series Round 4 of 4 종료 — R683.A 윈도우-라이프사이클 substrate + R683.B Splitter + DockPanel widget substrate + R683.C 첫 dock consumer + R679 ClickRouter cascade closure + 3건 R683.B substrate gap inline 청산; multi-window pro-tool authoring 정통 ground 도달; tear-off / dock-back round-trip end-to-end verified via 40+ assertion demo; AI-introspection-first 채널 (direct tear_off invoke) 우회 추가 — 헤드리스 RPC 환경에서 floating-window 입력 라우터 미초기화 문제 우회) = 북극성 가중 **~30-32%**. R684 (LayoutStyle::flex_basis 정통 substrate 확장) 완료 시 ~31-33% (Phase B ~62% — dock 패널 헤더 + splitter ratio 실측 정확도 도달).
...
【R685 plan】next round entry — **DockSurface topology composition substrate + 2nd dock consumer to trigger Rule-of-Three**. Single round, 4-5 atomic on top of R684 `14bf51b`.

(0) **`pinion_widget_paint::dock::DockSlot` enum + topology types** — new `#[non_exhaustive]` enum `DockSlot::{Left, Right, Top, Bottom, Center}` (mirror of VSCode / IntelliJ / Photoshop dock layouts). New `DockTopology` struct carrying a recursive `Split { orientation, ratio, slot_a: DockNode, slot_b: DockNode }` / `Leaf { panel_id, slot: DockSlot }` tree. Pure data — no External, no scene composition yet. ~10 unit tests pin the tree round-trips + serde-derived JSON I/O for AI-introspection. honest LOC: ~+200 net.

(1) **`DockSurface::view(topology, panel_views, theme) -> Scene`** — recursive walker over `DockTopology` that emits nested `view_splitter(left, right, ratio_signal, theme, style, dragging)` calls per `Split` node + `view_dock_panel(...)` per `Leaf`. Uses R684 `flex_basis(Px(0)) + flex_grow(ratio)` everywhere so the rendered split matches the declared ratio. ~6 unit tests pin the recursive composition (1-leaf / 2-leaf horizontal / 2-leaf vertical / 3-leaf nested / 4-leaf 2×2 grid / 5-leaf asymmetric). honest LOC: ~+250 net.

(2) **`hello-dock-panels-editor`** — 26th example binding, 2nd dock consumer per `[[abstraction-needs-second-consumer]]`. Asymmetric topology (main viewport + left outliner + right properties + bottom console + top toolbar = 5-pane Editor layout, mirror of every DCC / IDE shell). Triggers Rule-of-Three substrate lift for any shared dock helpers `hello-dock-panels` (R683.C) introduced inline; lifts go into `pinion_widget_paint::dock`. ~30 binding tests + `tools/demos/r685_editor_layout.py` (≥40 assertions across drag-to-resize / tear-off / dock-back / cross-panel select / topology persistence). honest LOC: ~+1200 net (composed app + demo density).

(3) **R683.C dock-back demo conversion** — `tools/demos/r683_dock_tear_off.py` section E (currently uses direct `invoke('tear_off', Null)` bypass) converted to drag-based using R684 atomic 3's first-paint finalize. Sequence: `scene/snapshot {window: 'torn-...', viewport: {w, h}}` primes the floating window's InputRouter via the first-paint gate → `scene/drag {window: 'torn-...', from_path: 'header#dock-handle', to_path: 'main-dock-target'}` works. R683.C carry cleared. honest LOC: ~+50 net (demo-only).

(4) **45-demo regression sweep ×3 + commit + Mnemosyne consolidation + SEED handoff**. Commit `feat(widget-paint): R685 §5.16 §5.41 §5.49 DockSurface topology + 2nd consumer`. Mnemosyne `entry_id = R685` + impact_refs `[5.16, 5.41, 5.49]` + R684 carry (transform-aware fragment cache key, wgpu damage consumer, DockSurface drag-to-reorganize substrate, Phase B widget catalog expansion) preserved + R686 axis (likely DockSurface drag-to-reorganize handles — drop targets on adjacent panel edges to swap / reparent within the topology).

honest LOC 예측: **R685 = +1700-2200 net** (substrate + 2nd consumer composed binding + dock-back demo conversion + demo + carry).

R685 후 가중 진척: ~31-33% → **~33-35%** (Phase B 25% × ~70% — recursive dock topology + drag-to-resize within editor layout; Phase D editor self-hosted natural land start point). Phase C entry (game-loop / 3D / assets / physics / audio / PBR) 가 R<700-900 range>에서 자연 도래 — 5-pane editor 패턴이 Unreal Editor 자체-호스팅 prerequisite 첫 step.

**R685 verification mandatory** (라운드 끝):
- 45-demo regression sweep PASS deterministic 3 consecutive runs (R660-R684 + R685)
- DockTopology serde round-trip pin (every variant serializes + parses back to identity)
- DockSurface recursive composition pin (5 different topology shapes via unit tests)
- 2nd dock consumer (`hello-dock-panels-editor`) demo PASS + Rule-of-Three substrate lift evidence (specific helpers identified + moved)
- R683.C dock-back demo section E converted to drag-based + PASS
- backward-compat — `hello-dock-panels` (1st consumer) bit-identical post-substrate-lift; single-window binding 18개 회귀 0
- 부채 surface 정직 받아들임 — DockSurface drag-to-reorganize (R686 carry), transform-aware fragment cache key carry (R682+1), wgpu damage consumer carry

**R684 verification (라운드 끝, completed)**:
- ✓ 44-demo regression sweep PASS deterministic ×3 consecutive runs (R660-R683.C + R684)
- ✓ cargo test --workspace PASS (3910 tests; R683.C baseline 3844 → R684 3910; +66 net incl. 7 substrate + 5 runtime + 1 dock + 2 splitter + 7 headless-RPC tests + rebuild counts)
- ✓ cargo clippy --workspace --all-targets --features pinion-runtime/vello clean under workspace `-D pedantic`
- ✓ todomvc R660 regression bisect end-to-end: pre-R684 PASS → naive atomic 3 finalize FAIL (`End jumps to All (last index): expected 2, got 1`) → root caused (`InputRouter::update_paint_scene` synthesises `PointerEnter` via `refresh_hover` for every active cursor, mutating RadioGroup statechart on every RPC dispatch) → fix gates the finalize on first-paint-only via `has_last_paint_scene_for_window()` so already-active windows do not pay the side-effect cost → PASS restored
- ✓ Atomic 1 honest narrowing: dock-panel header fix uses `Size::height_px` (cross-axis stretch) rather than the seed's prescribed `flex_basis + flex_grow` — flex_basis/flex_grow on the header would compete with the `content_wrapper`'s `flex_grow(1.0)` for the Column-parent Y-axis (header height would no longer pin to 28px). The splitter (atomic 2) is the canonical R684 flex_basis substrate consumer; dock header is the cross-axis-stretch fix only. Both axes share the same R683.C "1st consumer adoption" honest carry root cause.
- ✓ Mnemosyne entry R684 clean validate (T1 reject=0, GENERATED.md sync, atomic ledger entries=552 → 553, orphan_refs=5+0 frozen)
- ✓ 부채 surface 정직 받아들임 — R685 DockSurface topology composition substrate (next round) / R683.C dock-back demo conversion to drag-based (R685 carry — substrate-side first-paint gate is in place, demo-side scaffolding for `scene/snapshot` → `scene/drag` sequencing is a follow-up dev-experience round) / transform-aware fragment cache key carry (R682+1) / wgpu damage consumer carry

**R683.C verification (라운드 끝, completed)**:
- ✓ 44-demo regression sweep PASS deterministic ×3 consecutive runs (R660-R682 + R683.C; per-sweep total ~150s; r683_dock_tear_off.py 7.30s)
- ✓ cargo test --workspace PASS (65 suites, 0 failures); +14 ClickRouter substrate tests + +29 hello-dock-panels binding tests + +1 splitter Surface-fill test
- ✓ cargo clippy --workspace --all-targets --features pinion-runtime/vello clean
- ✓ R683.B substrate gap inline 청산 (3 건): (i) splitter Surface fill (BLACK clear leak-through 제거), (ii) DockPanelExternal InputRouter routing fix (composite tag → panel root tag + sub-index discriminator), (iii) DockPanelExternal direct tear_off invoke channel (headless RPC dock-back bypass)
- ✓ R679 DevTools cascade closure — ClickRouter substrate lift via Rule-of-Three; `hello-multi-window` MainWindowClickRouter removed + ClickRouter consumed; bit-identical visible behaviour (R679 demo PASS in 4.6s post-lift)
- ✓ Mnemosyne entry R683.C clean validate (T1 reject=0, GENERATED.md sync, atomic ledger entries=551 → 552, orphan_refs=5+0 frozen)
- ✓ 부채 surface 정직 받아들임 — R684 LayoutStyle::flex_basis 정통 substrate 확장 (header collapse + splitter ratio collapse 두 R683.B gap 의 textbook 정통 청산); headless-RPC floating-window paint cycle carry (R684 axis 3); R685 DockSurface topology composition substrate carry; transform-aware fragment cache key carry (R682+1)
