...
> **R703 land (2026-05-29, commit `ae39a48`)** — **SSOT 부채 청산** (사용자 "교과서적/SSOT?" 리뷰가 트리거 → grep+read 독립 감사로 R702 가 도입한 verbatim 중복 2건 발견 → feature 전 inline 청산, [[verify-seed-claims-audit-first]]). **발견**: (a) modal scrim backdrop 가 dialog.rs+drawer.rs 에 독립 재구현(0x66 alpha + scrim_color() + absolute-full-window dim-container) — framework primitive 2-copy = SSOT 위반. (b) composite-button 헬퍼 4종(button_a11y_state IDENTICAL + read_button_state/read_button_focused/button_scene IDENTICAL)이 hello-dialog↔hello-drawer 복제. **청산**: (1) 신규 `crates/pinion-widget-paint/src/scrim.rs` — `M3_SCRIM_ALPHA` const + `scrim_fill(alpha)` + `scrim_backdrop(tag,fill,viewport,flex,align,justify,panel)`; DialogStyle/DrawerStyle 가 delegate, view_dialog/view_drawer 가 scrim_backdrop 호출(dialog=center, drawer=edge-anchored). (2) `read_button_state`/`read_button_focused`/`button_a11y_state` → `pinion_widget_paint::button` 로 lift(crate 가 이미 pinion-a11y dep). `button_a11y_state`(ButtonState→AccessState)가 이제 single home — derive 가 single-node 에 emit 하는 것과 동형, a11y mapping 2-copy = a11y 버그 위험. **원칙적 deferral**: `button_scene`(filled-tonal paint-compose)은 per-example 유지 — button PAINT 헬퍼라 Rule-of-Three(use_hover_progress R686.C 가 3 consumer 에서 lift 한 선례) 적용, 2 consumer 에서 style opinion 을 substrate 에 굳히는 위험 회피 → 3rd consumer 시 lift(정직 carry). **검증**: behaviour-preserving — 75 suite green(widget-paint 347 = +4 scrim +2 button-helper), clippy `-D pedantic` clean, r693+r702 demo PASS(rebuilt release binary, bit-identical scene). Mnemosyne `R703`(585; T1 +0; RT 1/1; sync; impact [5.16,5.40]). **환경**: `-j2` 필수(OOM). (R702 land 상세 — hello-drawer 위젯 자체 — 는 git `36285a2` + GENERATED.md.)
>
> **다음 세션 진입**: `load` 단독 입력. **R704 = Phase B widget-catalog breadth 계속**. 후보: Table (Model/View + virtualization, substrate-first 또는 documented row-limit) / DatePicker (calendar grid + date type, substrate 0) / ColorPicker (2D pad + HSV math, substrate medium). 진입 시 [[round-direction-auto-select-no-ask]] 대로 northern-star 정렬 자동 선택 + 후보 substrate 전제 grep 재감사([[substrate-incompleteness-signal]]). 큰 substrate 필요하면 substrate 라운드 먼저(substrate-first ordering).
>
> R703 가중 진척: Phase A 97% + Phase B 25% × ~95.8% + Phase C 35% × ~12% = 북극성 가중 **~39.2%** (SSOT 청산 라운드 — 진척 수치 불변, 구조 건전성↑).
...
- R690 Tabs ✓ / R691 Menu ✓ / R692 Toolbar ✓ / R693 Dialog ✓ / R694 focus-ring substrate ✓ / R695 Tooltip ✓ / R696 Disclosure ✓ / R697 Accordion(multi-open) ✓ / R698 widget_state_name! SSOT sweep ✓ / R699 widget_event_name! from_name + parse_*_event SSOT sweep ✓ / R700 DisclosureGroup single-expand coordinator ✓ (substrate-first) / R701 hello-accordion-single ✓ (single-open accordion = R700 consumer, hello-radio-group mirror) / R702 hello-drawer ✓ (modal nav drawer = R693 modal substrate 2nd consumer + scrim light-dismiss) / R703 SSOT 청산 ✓ (modal scrim primitive lift + composite-button 헬퍼 lift)
- R704+ = Table (Model/View, virtualization substrate-first 또는 documented row-limit) / DatePicker (calendar grid + date type, substrate 0) / ColorPicker (2D pad + HSV math). 진입 시 class + substrate 전제 재감사
