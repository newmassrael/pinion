...
> **R705 LANDED (`0b8e5db`): producer-parity + scroll-aware focus ring.** ★★★ [[r705-producer-parity-focus-ring]]
>
> **§2 #7 restored.** `scene/snapshot from:paint` 이제 저장된 `InputRouter::last_paint_scene` 를 그대로 직렬화 — `render_window` 가 GPU 에 그리는(`to_vello_cached(&scene)`) 바로 그 `Scene` 객체를 router 에도 저장(`finalize_frame(scene)`)하므로 **introspection == 화면 by construction** (두 렌더러가 우연히 일치가 아님). CoreShell split-borrow `scene_mut_and_last_paint_for_window` → `DispatchContext.with_last_paint_scene` wiring; never-painted(headless) 창만 producer fallback.
>
> **focus-ring 위치 버그 청산.** 링이 focus 위젯의 scroll-LOCAL rect 로 만들어졌는데 top-level(window-absolute)로 그려져 → `Scene::Scroll` 안 위젯(listbox row, tree row, fold 아래 컨트롤)은 수십 px 어긋남. 신규 SSOT `Scene::rect_for_tag_absolute`(pinion-core: scroll-offset 변환 + viewport clip)로 해결, `pinion_rpc::find_rect_by_tag`(click/drag resolver)도 여기 위임 — 한 resolver 라 input 착지점과 ring 위치가 drift 불가.
>
> **검증이 tautological 이었던 이유 + 정통 grounded TC.** demos 가 ring vs widget rect 를 **같은 scroll-local 공간**에서 비교 → 화면 위치와 무관하게 통과. 신규 `rpc_verify.assert_focus_ring_concentric` 가 WINDOW-ABSOLUTE rect 를 독립 재계산; `r705_focus_ring_placement.py` 가 24 GUI 예제 31 ring + scrolled-row 케이스 grounded 검증. **라이브 ffmpeg x11grab 대조**(dialog Cancel ring; scrolled listbox "Fig" row) == from:paint 확인. **from:paint 단독 신뢰 금지는 여전 유효** ([[introspection-from-paint-not-screen]]) — R705 가 from:paint==화면을 보장하므로 이제 신뢰 가능하나, 라이브 대조가 그 보장의 증명.
>
> **dirty-on-mutation (lag half).** reactive-Signal-only 변경(listbox/tree focused-row, cross-window selection)은 SCXML state_change 가 없어 redraw 를 안 부름 → from:paint stale. handled `apply_key`(`try_apply_key`) + drained intent(`handle_tail`)에서 redraw arm + **모든** painted window 재저장(`painted_window_sizes`)으로 해결. revision-delta 아님 — benign click 이 revision 올려 R682 cache churn. 이로써 8개 demo regression(r673-r679, r683) 청산. r683 "실패"는 stale binary 였음([[rpc-subprocess-release-default]]).
>
> **검증:** cargo test 76 ✓ / clippy clean ✓ / demo sweep 58/58 (R704 제외). live: dialog + scrolled-listbox ffmpeg.
>
> **R706 = 다음 최우선: R704 datepicker 마무리 (이번 세션 git-stash 됨 — `git stash list` → `git stash pop`).**
> datepicker 라이브러리(pinion-core widget + pinion-widget-paint + a11y Grid/GridCell/ColumnHeader/Row roles + hello-datepicker example)는 빌드되나 demo `r704_datepicker.py` 가 실패: (a) `scene/simulate` final-projection **RollbackFailed**, (b) 날짜 **클릭이 active-descendant(`focused_day`)를 안 옮김** → 링이 selection 과 분리(WAI-ARIA grid 는 클릭이 active-descendant 도 이동해야). 이후 month-crossing arrow nav(R704 carry). **이제 from:paint==화면이라 ring 을 신뢰성 있게 검증 가능** — `r705_focus_ring_placement.py` 식 grounded 대조 + 라이브 ffmpeg 로 datepicker grid ring 확인. stash pop 후 진행.
...
- focus-ring 은 **이미 그려짐**(active-descendant cell): view_datepicker `focused` 인자를 focused_day projection 으로 wire → 가시 ring + AT aria-activedescendant + RPC `focused_day` query 3자 일치. (R694 catalog-wide focus-ring carry 는 이 위젯엔 비적용.)
...
- R690 Tabs ✓ / R691 Menu ✓ / R692 Toolbar ✓ / R693 Dialog ✓ / R694 focus-ring substrate ✓ / R695 Tooltip ✓ / R696 Disclosure ✓ / R697 Accordion(multi-open) ✓ / R698 widget_state_name! SSOT sweep ✓ / R699 widget_event_name! from_name + parse_*_event SSOT sweep ✓ / R700 DisclosureGroup single-expand coordinator ✓ (substrate-first) / R701 hello-accordion-single ✓ (single-open accordion = R700 consumer, hello-radio-group mirror) / R702 hello-drawer ✓ (modal nav drawer = R693 modal substrate 2nd consumer + scrim light-dismiss) / R703 SSOT 청산 ✓ (modal scrim primitive lift + composite-button 헬퍼 lift) / R704 hello-datepicker ⚠ WIP-uncommitted (inline month-calendar grid; WAI-ARIA grid role family 추가 = Table 의 a11y 토대; RadioGroup-mirror 2D single-select + roving active-descendant; substrate-first 로 Table 연기) — 미커밋, focus-ring introspection 결함 + simulate RollbackFailed 미해결(상단 R705 블록 참조)
- R705 = **focus-ring introspection RFC 최우선**(상단 블록); 이후 ColorPicker (2D HSV pad + hue slider, Slider 재사용 검토) / DatePicker month-crossing arrow nav 청산(R704 deferred, 작음) / Table (virtualization substrate-first; R704 grid role 이 a11y 토대 — DatePicker=grid-role 1st consumer, Table=2nd + virtualization). 진입 시 class + substrate 전제 재감사
