...
> **R700 land (2026-05-29, commit `16cede1`)** — **`DisclosureGroup` single-expand coordinator (substrate-first)** = single-open accordion 의 framework mutual-exclusion 기반. **R47-class**: "한 섹션 열리면 열려있던 섹션 collapse" 는 framework-owned (application loop 금지), RadioGroup(R51.15) 과 동형. **핵심**: `crates/pinion-core/src/widgets/disclosure_group.rs` 신규 — RadioGroup mirror 이되 더 단순(`expanded: Option<usize>` 단일, **roving `focused_index` 無** — APG accordion 은 헤더별 Tab stop + arrow=focus-only 라 그 focus 모델은 binding 의 focusable_tags+focus_request 에 R697 처럼 남김; coordinator 는 expand-exclusion 만 소유). `send(i,ev)`: section i drive 후 collapsed→expanded edge 면 sibling 전부 `set_expanded(false)`+`expanded=Some(i)`, expanded→collapsed edge 면 `None`. `set_expanded(Option)` model-driven(slot-assign, intent 無). `WidgetTransition::detect` 가 변화 시 `"expanded"` intent(Int idx, collapse-to-none 시 Null — send 재활성으로 도달, RadioGroup 과 달리 clear 가 send 로 가능). `DisclosureGroupExternal` = RadioGroupExternal mirror RPC 표면: query count/expanded_index/state.<i>/expanded.<i>, intervene expanded_index(Int/Null + OutOfRange/TypeMismatch/ReadOnly), invoke send `"<i>:<EventName>"`(R660 parse_send_payload + R699 `DisclosureEvent::from_name`). widgets/mod.rs 등록. **substrate-first 정당성**: RadioGroup R51.15 가 logical group 을 consumer 보다 먼저 land 한 선례 — "mutual exclusion 이 framework debt 라 먼저". **검증**: 12 신규 unit test 가 §5.12 RPC 계약 전부(single-open 강제 = open-switch collapses previous + reactivate→none, typed API + invoke 양경로; set_expanded model + OOB panic; expanded-intent open/switch/close=Int+Null + idempotent silence; query/intervene/invoke happy+모든 reject path incl. internal DisclosureActivate + malformed; schema). `cargo test --workspace` -j2 73 suite green(core 1514 = 1502+12), clippy `-D pedantic` clean. Mnemosyne `R700`(ledger 581→582; T1 +0; RT 1/1; GENERATED sync; impact [5.38]). **환경**: `-j2`(CARGO_BUILD_JOBS=2) 필수(OOM).
>
> **다음 세션 진입**: `load` 단독 입력. **R701 = R700 consumer 반쪽** — single-open accordion **GUI 예제 + RPC demo(≥30 assert)** = substrate-first split 로 미룬 visible deliverable. 설계: `examples/hello-accordion-single` 를 **hello-radio-group(962 LOC) mirror** — 단일 `DisclosureGroupExternal` 를 PRIMARY_TAG 에 등록, view 가 per-section state 를 `query` 로 읽고, row click 이 `invoke("send","<i>:<Event>")` (composite-tag [[r51-42-composite-tag-protocol]]), 헤더별 Tab stop + arrow-roving 은 `focus_request`(R697 동형). a11y(WidgetA11y composite) + renderer + Cargo.toml + build.rs 포함. demo: `tools/demos/r701_accordion_single.py` 가 expanded_index 단일성 + open-switch-collapse + close 를 ≥30 assert. **R701 후** Phase B breadth 재개(Drawer/Table/DatePicker/ColorPicker).
>
> R700 가중 진척: Phase A 97% + Phase B 25% × ~95.3% + Phase C 35% × ~12% = 북극성 가중 **~39.0%** (framework coordinator substrate; breadth 는 R701 consumer 시 반영).
...
- R690 Tabs ✓ / R691 Menu ✓ / R692 Toolbar ✓ / R693 Dialog ✓ / R694 focus-ring substrate ✓ / R695 Tooltip ✓ / R696 Disclosure ✓ / R697 Accordion ✓ / R698 widget_state_name! SSOT sweep ✓ / R699 widget_event_name! from_name + parse_*_event SSOT sweep ✓ / R700 DisclosureGroup single-expand coordinator ✓ (substrate-first, single-open accordion 기반)
- R701 = R700 의 GUI consumer (single-open accordion 예제 `hello-accordion-single`, hello-radio-group mirror) + RPC demo ≥30 assert = substrate-first split 의 visible-deliverable 반쪽. 그 후 R702+ = Table (Model/View, virtualization substrate-first) / Drawer (overlay-dismiss substrate-first, menu click-outside R691 carry 동시 청산) / DatePicker / ColorPicker. 진입 시 class + substrate 전제 재감사
