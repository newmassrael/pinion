...
【R686 plan】 next round entry — **DockSurface drag-to-reorganize handles**. Phase B/D editor 정통 진입의 다음 step — adjacent panel edge 위 drop targets 가 swap / reparent / split-and-insert gestures 를 지원하면 user 가 editor layout 을 mouse 만으로 자유롭게 재배치 가능. 5-pane editor 가 "사용자가 직접 customise 한 layout" 으로 전환되는 첫 라운드. 5-6 atomic.
