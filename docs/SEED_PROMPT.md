...
> **R682.A land (2026-05-27, commit `8d01de1`)** — 4-axis paint-pipeline rewrite series Round 3 of 4 (axis 4 substrate atomics 0-3 of 5). Honest split per the R670.A/B precedent: atomics 4 (todomvc 100-row stress consumer) + 5 (R682 demo + 23-demo sweep ×3 + Mnemosyne entry consolidation) carry to **R682.B** next session. Substrate landed this session covers the full §5.16 paint-fragment cache contract end-to-end (per-Container structural hash, per-window Vello fragment cache with mark-and-sweep eviction, damage rect propagation, GUI-agnostic stats publish/getter pair). **Atomic 0** — `pinion-core::ContainerNode` gains `paint_hash: Cell<Option<u64>>` per-paint-pass memoised structural hash + `Scene::paint_hash()` recursive over every variant + `Scene::is_cacheable_for_paint()` cacheability predicate. `External` / `ImmediateModeNode` return fixed `PAINT_HASH_UNCACHEABLE` sentinel; `Effect` returns distinct `PAINT_HASH_EFFECT_SENTINEL`. Hash excludes paint-irrelevant fields (`tag` / `aria_label`); covers `rect` / `style` / `layout` / child-hash recursion in declaration order. `Rect` / `Size` / `SizeValue` derive `Eq + Hash`; `LayoutStyle` manual `Hash` impl via `f32::to_bits` for the `flex_grow` axis. **Atomic 1** — `pinion-runtime::paint_adapter::FragmentCache { fragments: HashMap<u64, VelloScene>, seen_this_paint: HashSet<u64>, hits/misses/paint_count counters }` + `to_vello_cached(scene, fill_hook, text_cache, fragment_cache, out)` brackets the encoder walk with `begin_paint` / `end_paint` so unreached entries evict at sweep. Cache probe gates at each `Scene::Container` boundary reached under accumulated `Affine::IDENTITY` AND `is_cacheable_for_paint() == true`; hit → `vello::Scene::append` the cached fragment + skip recursion; miss → encode the entire subtree into a fresh sub-`VelloScene` at IDENTITY + append + install. Non-cacheable Containers recurse via `to_vello_cached_inner` so nested cacheable subtrees still participate (the §2 #4 killer use case: an `ImmediateModeNode` sibling triggers `V::view` re-runs every paint but the retained widget subtree skips re-encoding). `Scroll` threads non-IDENTITY child transform through; descendants under `Scroll` skip the cache (R682+1 carry — container-local-coord refactor unlocks transform-invariant cache keys). Shell wiring: `WindowSlot` owns `FragmentCache` per slot; `AppShell::render_window` now calls `to_vello_cached` instead of `to_vello`; `WindowSlot::build` factored to keep `resume_spec` under the clippy line cap. **Atomic 2** — `Rect::union` (zero-area-set short-circuits) added to `pinion-core::scene`; `FragmentCache.damage_acc_this_paint` accumulates miss-Container rects per paint; `end_paint` publishes the accumulator into `last_damage_region`; the getter returns the most-recent-paint damage region (`None` when 100% cache hit). Production GPU consumer (`wgpu::SurfaceTexture` partial-blit / `winit` damage-rect coordination) is a follow-up round carry — the current per-window paint cycle still resets the entire `vello::Scene` and submits a full surface every frame. **Atomic 3** — GUI-agnostic `pinion-shell::FragmentCacheStats { hits, misses, paint_count, entries, last_damage_region }` with `hit_rate()` helper; `ShellCore::publish_fragment_cache_stats` + `fragment_cache_stats_for_window` + `fragment_cache_stat_windows` per-window observability pair. `AppShell::render_window` captures the per-WindowSlot cache snapshot post-paint and publishes via `spec_id` key. Re-exported from `pinion-shell::lib` so RPC + tests + demos consume without crossing the `vello::Scene`-bearing `paint_adapter::FragmentCache` directly. **Verification**: 52 new R682 unit/integration tests (23 `pinion-core::scene` + 13 + 8 `pinion-runtime::paint_adapter` cache + damage rect + 8 `pinion-shell::tests::dispatch_core::r682_fragment_cache_stats_substrate`) PASS; workspace ~3749 tests pass (R681 baseline 3697 → +52 net); clippy clean under workspace `-D pedantic` baseline; 42-demo regression sweep PASS deterministic (zero regression on cached path; the §2 #4 `hello-immediate-mode-canvas` killer case verified — retained widget tree subtrees cache + immediate-mode driver paints fresh per tick). Mnemosyne entry `R682.A` lands with `impact_refs = [§5.16, §5.41]` + R682.B carry recorded; `[[publishable_override_ledger]]` + `[[orphan_ledger]]` rows added to `mnemosyne.toml` to clear the §5.46 audit-half typo (no `§5.46` section exists; substrate factually lives in `§5.16` paint pipeline + `§5.41` render loop). **R682.B is the next session** — atomic 4 (todomvc 100-row stress consumer + filter-mode change re-encode verify via `FragmentCacheStats` RPC introspection) + atomic 5 (`tools/demos/r682_dirty_subtree_cache.py` with ≥30 assertions + 23-demo regression sweep PASS ×3 + Mnemosyne entry consolidation + SEED handoff).
>
> **다음 세션 진입**: `load` 단독 입력. SEED 의 R682.B plan 절 (atomic 4 + 5 — todomvc 100-row stress consumer via [[abstraction-needs-second-consumer]] 2nd consumer of `FragmentCacheStats` substrate + R682 demo + 23-demo sweep) 자동 진행. 모든 atomic 은 "비용 무관 + 장기 textbook canonical" 원칙 따라 작성 — MVP / shortcut 금지. R682.B 종료 시 R682 가 4-axis paint-pipeline rewrite series Round 3 of 4 의 진정 종료점.
>
> R682.A 가중 진척: Phase A 97% + Phase B 25% × ~33% + Phase C 35% × ~5% (4-axis paint-pipeline rewrite series Round 3 of 4 substrate complete; §2 #4 dual-execution model의 immediate-mode coexistence가 retained tree cache hit 으로 비용 사실상 0; per-Container 구조 해시 + Vello fragment cache + damage rect + 관찰성 stats 모두 wire; Phase C entry prerequisite chain 3/4 advanced) = 북극성 가중 **~22-24%**. R682.B + R683 series 완료 시 ~30-34% (R682.B가 100-row stress 로 dirty cache 실측 검증; R683 dock-panel runtime 이 Phase B/D 진정 진입).

【R682.B plan】next round entry — **4-axis paint pipeline rewrite series Round 3 of 4 continuation** (atomics 4 + 5 of axis 4 substrate). Single commit, 2 atomic.

(4) **first dirty-cache consumer — todomvc 100-row stress binding**. Per the original R682 plan: "todomvc 의 100-row long list (filter 변경 시 변경된 row 만 재encode 검증)". Implementation options + decision in R682.B opening: (A) extend `examples/todomvc` with `PINION_TODOMVC_SEED_N=100` env var that seeds 100 `TodoItem` rows on first paint, then use the existing 3-filter (`All` / `Active` / `Completed`) Mode switching to drive filter-change paints + observe `FragmentCacheStats` via RPC; (B) new `examples/hello-long-list` dedicated binding (cleaner separation but +200 LOC overhead). Prefer (A) for the 2nd-consumer textbook trigger of `FragmentCacheStats` ([[abstraction-needs-second-consumer]] gate fires; substrate observability ratifies). RPC wire: add `cache_stats` field to `scene/snapshot` response OR new `scene/cache_stats` RPC method — pick based on `pinion-rpc::dispatch` audit (the snapshot-field path keeps the RPC surface count stable). 8-12 binding tests pin: 100-row seed → 100 row fragments + 1 root fragment installed first paint; filter `All → Completed` → only completed-row fragments hit, incomplete-row fragments evict via mark-and-sweep; cache `hit_rate` ≥ 0.85 after 3 paints of stable filter; damage region `None` after 2nd identical paint. honest LOC: ~500-700 net (todomvc seed extension ~150 + RPC cache-stats surface ~120 + binding tests ~300 + cache-aware fragment Cell-reset pin in scene test if R682.A discovers any cross-frame re-use bug — none surfaced in R682.A but the 100-row stress is the empirical proof).

(5) **R682 demo + 23-demo regression sweep + commit + Mnemosyne consolidation + SEED handoff**. `tools/demos/r682_dirty_subtree_cache.py` (≥30 assertions across A-H sections): A substrate sanity (cache_stats RPC surface non-null after first paint) / B 100-row warmup (101 entries + 101 misses + 0 hits) / C steady-state hit rate (3 consecutive paints same scene → hit_rate ≥ 0.99) / D filter-driven per-row eviction (filter `All → Completed` → only completed rows hit, 50 incomplete-row entries evict via mark-and-sweep) / E damage region (multi-mutation paint → union covers both mutated rects) / F immediate-mode coexistence carry-back (re-run R681 canvas demo through cached path → still PASS with retained-tree cache hits) / G full-hit no damage (steady-state paint → `last_damage_region == None`) / H `FragmentCacheStats` `paint_count` monotonic across 5 paints. 23-demo regression sweep PASS deterministic across 3 consecutive runs (R660-R681 + R682). Commit `feat(runtime): R682.B §5.16 §5.41 paint-fragment cache stress consumer + demo` ≤ 72 bytes title + ≤ 3 bullets ≤ 72 bytes each. Mnemosyne `entry_id = R682.B` (monotonic after R682.A) + impact_refs `[5.16, 5.41]` (no §5.46 typo this time) + R682.B carry resolved + R683 axis 1 dynamic dock series entry. SEED record updated with R682 land summary (consolidating R682.A + R682.B) + R683 plan handoff.

honest LOC 예측: **R682.B = +800-1200 net** (todomvc 100-row seed + RPC cache-stats wire + binding tests + R682 demo + sweep + Mnemosyne entry consolidation). Smaller than R682.A's +1995 because the substrate is already in place.

R682.B 후 가중 진척: ~22-24% → **~24-26%** (Phase B 25% × ~37% — 4-axis paint-pipeline rewrite series Round 3 of 4 종료; §2 #4 dual-execution model의 immediate-mode coexistence 실측 검증; per-row dirty-cache benefit 정량화).

**R682.B verification mandatory** (라운드 끝):
- 23-demo regression sweep PASS deterministic 3 consecutive runs (R660-R681 + R682)
- 100-row todomvc filter change → only changed rows re-encode (hit-rate jump on filter-stable paints)
- `FragmentCacheStats` RPC surface returns typed snapshot per window
- backward-compat — 단일 윈도우 binding 18개 + multi-window binding 회귀 0
- 부채 surface 정직 받아들임 — transform-aware cache key (Scroll-content + reflow cache hits) carry, R683 axis 1 dynamic dock substrate carry, wgpu surface invalidation consumer carry

**R682.A verification (라운드 끝, completed)**:
- ✓ 42-demo regression sweep PASS (zero regression on cached path; §2 #4 `hello-immediate-mode-canvas` killer case verified)
- ✓ 23 atomic-0 unit tests (paint_hash determinism / mutation propagation / sentinel distinctness / Scroll recursion / Cell memoisation)
- ✓ 13 atomic-1 unit tests (cache hit/miss/sweep / nested-cacheable-under-uncacheable / clear)
- ✓ 8 atomic-2 unit tests (Rect::union semantics / damage region first-paint + reset + multi-miss union)
- ✓ 8 atomic-3 integration tests (publish round-trip / per-window independence / latest-wins / hit_rate math)
- ✓ Mnemosyne entry R682.A clean validate (publishable_override_ledger + orphan_ledger rows for §5.46 audit typo registered in mnemosyne.toml)
- ✓ 부채 surface 정직 받아들임 — atomic 4 + 5 → R682.B; transform-aware cache key → R682+1 carry; wgpu surface invalidation → future round carry
