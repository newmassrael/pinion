...
> **R689 land (2026-05-28, commit `187eb1d`)** — `load` 진입 후 사용자의 "hack/smell 없이 교과서적인가? SSOT 지켜지나? 비용 무관 장기 정통 구조인가?" 감사 요청에 대한 **정직한 후속**. `[[verify-seed-claims-audit-first]]` 대로 R688/R688.A 의 `reconcile_externals` 를 grep+read 독립 감사 — R688.A SEED 가 **Finding 3** 으로 "documented tradeoff (smell 아님)" 분류한 per-frame `create_extra_externals` 재실행이 **실제로는 inline 청산해야 할 self-inflicted smell** 임을 확인 (방금 land 한 라운드의 자기분류조차 독립 감사로 뒤집힘). **근거 4가지**: (1) `finalize_frame_for_window` (winit 매 paint frame) 가 `reconcile_externals` 무조건 호출 → 27 binding 전부 매 프레임 factory 재실행 = N throwaway `Box<dyn External>` alloc. (2) 더 심각: factory 가 boot-seeding **부작용** 보유 (settings-panel `intervene("value",...)` + `use_theme().set_mode(Dark)`) — 매 프레임 재실행 = boot 전용 seeding 을 매 프레임 재실행 (단순 alloc 낭비 아닌 correctness fragility). (3) 프로젝트 원칙 "외부 의존만 honest carry 정당" → self-inflicted per-frame alloc 은 carry 부적격, "documented" 해도 정당화 ×. (4) grep 으로 dynamic external set 보유 binding = editor (`hello-dock-panels-editor`, `Signal<DockTopology>` walk → `reorg-split-N` mint) **단 하나**; 나머지 26 (settings-panel 6-channel=const count / todomvc composite-tag singleton / multi-window·dock-panels·listbox 고정 tag) 전부 static 확인. **청산**: `WidgetCore::external_set_is_dynamic() -> bool` capability flag (default `false`) 신설 → `CoreShell::reconcile_externals` 첫 줄 `if !V::external_set_is_dynamic() { return; }` gate. static binding (26/27) 은 literal early-return (0 alloc / 0 factory 재실행 / boot-seeding 은 `CoreShell::new` 1회만 — 더 correct). editor + R688 `ReconcileFixture` 만 `true` opt-in. **설계 근거**: capability flag idiomatic (initial_size_strategy/windows trait hook 선례); generation-counter dirty-gate 는 evidence-first 미래 후보 (`[[abstraction-needs-second-consumer]]`). **검증**: `cargo test --workspace` exit 0 (pinion-runtime 266→267, 신규 `r689_static_set_binding_skips_factory_rerun`), clippy `-D pedantic` clean, **46/46 demo sweep PASS** (behavior-preserving), Mnemosyne `R689` (ledger 564→565; T1 reject=0; round-trip 1/1; impact_refs [5.16, 5.35, 5.45]). 메모리: `[[round-direction-auto-select-no-ask]]` 신설.
>
> **다음 세션 진입**: `load` 단독 입력. R690 = Phase B widget catalog 진입 — **Tabs widget 권장** (dock editor 와 tabbed-panel 합성 = canonical DCC/IDE pattern; northern-star "Unreal-class editor self-hosted" 최강 정렬) 또는 Menu widget. mouse drag-session UX (R687 carry a) 도 후보.
>
> **R690 설계 제약 (R689 감사 발견, mandatory)**: Tabs = "select 1 of N" = **RadioGroup 의미와 동일** → 새 SCXML state machine 작성 **금지**; settings-panel nav rail 선례대로 `RadioGroupExternal` 재사용 (`[[abstraction-needs-second-consumer]]`). R690 추가물: (1) `AriaRole::{TabList, Tab, TabPanel}` 3 variant (accesskit `Role::{TabList, Tab, TabPanel}` lower + aria_name + action set) — role.rs R673 Tree/TreeItem 패턴 mirror, (2) `pinion-widget-paint::tabs` (horizontal tab-strip M3 paint + active-tab indicator + `TabsStyle::m3_default`) — tree_view.rs mirror, (3) `examples/hello-tabs` binding (RadioGroupExternal(N) selection + view_tabs + per-tab content + WAI-ARIA Left/Right/Home/End roving) — hello-tree-view R673 mirror, (4) `tools/demos/r690_tabs.py` + cross-layer 테스트. R671/R673 TreeView 라운드와 동형 (~600-800 LOC).
>
> R689 가중 진척: Phase A 97% + Phase B 25% × ~82% + Phase C 35% × ~12% = 북극성 가중 **~36-38%**.
...
Phase B widget catalog cascade (R690+):
- R690 = Tabs (RadioGroup 재사용; 위 R690 설계 제약)
- 이후: Menu / Dialog / Toolbar / Table / Tooltip / Drawer / Accordion / DatePicker / ColorPicker (1라운드 1위젯 또는 small pair)
