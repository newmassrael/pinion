...
【R679 plan】 next round entry — single commit, 4 atomic. DevTools cascade continuation — bidirectional select arc completion:

(0) **`pinion_widget_paint::devtools` main-window click router** — currently the cross-window selection arc is **one-way** (inspector tree row click → V::update reducer → `use_selected_path` Signal → main window highlight wrap). R679 closes the loop: a main-window click on any tagged scene node also writes the corresponding path into `use_selected_path` so the inspector tree row paints the M3 focus state-layer in lockstep. Substrate helper: new `pinion_widget_paint::devtools::path_for_paint_hit(paint_scene, x, y) -> Option<String>` — given a hit position in the painted scene, walks the layout tree and emits the path-stable address (Container[tag] / Type[nth-of-type]) that matches what `scene_to_tree_item` would have produced. Reuses `scene_type_name` / `scene_tag` / `scene_child_path_segment` substrate helpers. 6-10 unit tests covering tagged button hit / untagged Text hit / root background hit / hit miss returns None / depth-first deepest-tag-ancestor semantics (W3C canonical, matches InputRouter `resolve_hover_tag`). Estimated LOC: +200-300 substrate.

(1) **hello-multi-window MainWindowClickRouter External + main-window invocation wire** — main window currently has only the `ButtonExternal` (the button widget). R679 adds a new `MainWindowClickRouter` External registered as a 2nd `ExtraExternal` under a new main-window tag `main_window_click_router` (or similar). When a click lands on a tagged path in the main paint scene, the External's invoke arc records the path and emits a new `main_window_click` intent. V::update reducer routes the intent into the `use_selected_path` Signal. Open design question — main-window click might **deselect** instead of select if the click is on the background — needs scope-time decision (mirror VS Code outliner: click-empty deselects). Estimated LOC: +250-350 binding.

(2) **`view_inspector` reads selected_path through TreeViewFocus** — already in place at R675; verify still works after the new cross-arc. Two-way demo: click main button → inspector tree row paints focus state-layer; click inspector tree row → main button paints Error red wrap. The dual-arc forms the canonical DevTools select bridge (Chrome's "select element in page → flash element in Elements panel" arc, mirrored bidirectionally). Estimated LOC: ~+50 verification only (no new code expected).

(3) **r679 demo + 19-demo sweep + commit + Mnemosyne** — `tools/demos/r679_devtools_bidirectional_select.py` (≥35 assertion): (A) baseline both Signals None; (B) inspector click → main wrap appears + selected_path = path; (C) main click on button → selected_path = same path + inspector focus state-layer paints on that row; (D) AI-driven scene/click {window:"main", path:"main_btn"} produces same observable Signal mutation; (E) bidirectional sync stress — alternating main/inspector clicks on different tagged nodes preserve invariant (latest click wins); (F) background main click → deselect (selected_path → None) or no-op (design decision pinned in atomic 1). 19-demo regression sweep PASS deterministic (R660-R678 + R679). Commit `feat(widget-paint): R679 §5.16 §5.49 §5.50 bidirectional select`. Mnemosyne entry_id=R679 + impact_refs [5.16, 5.49, 5.50] + carry (R680 2nd DevTools binding consumer / R681 pinion-devtools crate skeleton / R682+ multi-select model).

honest LOC 예측: **R679 = +900-1400 net** — (0) substrate +250 / (1) binding +300 / (2) verify-only ~50 / (3) demo +400 + SEED + Mnemosyne entry +120-200.

R679 후 가중 진척: ~15-17% → **~17-18%** (Phase D 35% × ~5-6% — DevTools cascade reaches **dual-arc parity**; bidirectional select is the most-canonical Chrome/Firefox DevTools affordance; substrate `path_for_paint_hit` matures the devtools substrate's hit-test surface).

**R679 verification mandatory** (라운드 끝):
- 19-demo regression sweep PASS deterministic (R660-R678 + R679)
- path_for_paint_hit semantics matches the inverse-walker invariant of find_node_at_path — i.e. `find_node_at_path(scene, path_for_paint_hit(scene, x, y).unwrap()).is_some()` for every successful hit
- bidirectional sync invariant — clicking node X on either window settles both Signal + visible-wrap to X; no oscillation under rapid alternating clicks
- background main click design decision pinned (deselect vs no-op) with explicit test coverage
- 부채 surface 정직 받아들임 — 2nd DevTools binding R680, pinion-devtools crate skeleton R681, multi-select model R682+
