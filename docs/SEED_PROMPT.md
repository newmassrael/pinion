...
> **이번 세션 LANDED: R704 (`ee3534e`) + R705.1 (`4dd6ce8`) + R706 (`0f9fe96`) + R706.1 (`7fba149`).** ★★★
>
> **R706 = paint fragment-cache "direct-draw-after-append" rasterize 결함 root-cause+fix.** 증상: hello-datepicker focused_day=N 이면 from:paint scene 은 링을 cell#N 에 정확히 박는데 GPU 는 +1열(+42px) 오른쪽. **계측으로 확정**: 링은 (32,163) identity transform 으로 *정확히* stroke 되는데 `to_vello_cached` 경로에서만 화면이 어긋남 → `to_vello`(uncached) 우회 시 정상 → **R682 fragment cache 가 원인**(SEED 의 "ring Box 가 mis-raster" 추측은 방향만 맞고 기전은 틀림). **진짜 원인: vello 0.6 `Encoding::append` 이 child 의 force-transform/style flag 복사(`self.flags=other.flags`)+transform stream 확장 → append *뒤* 에 같은 vello scene 으로 direct fill/stroke 하면 stale encoder state 재사용.** R705 가 ring 을 cached grid append *뒤* 에 stroke 되는 overlay Box 로 만들며 충돌. **fix: `to_vello_cached_inner` 재작성 — `out` 은 append 만 수신(모든 direct-draw 노드는 fresh sub-scene 에 encode 후 append; direct draw 는 항상 자기 scene 의 첫 op, append-뒤-direct 금지).** PINION_SCREENSHOT 도 `to_vello`→`to_vello_cached` 전환(live-render parity). pixel-only 결함 — from:paint 못 잡음 [[introspection-from-paint-not-screen]]. 가드: `tools/demos/r706_focus_ring_pixel.py`(라이브 ffmpeg 픽셀; display/ffmpeg/PIL 없으면 graceful skip) + **R706.1 결정적 wgpu 회귀 테스트**(`pinion-shell::headless_screenshot::tests::r706_cached_render_frames_*` #[ignore]) — 실제 `view_datepicker` 장면을 production pipeline(compute_layout + inject_focus_ring)로 짓고 to_vello_cached+HeadlessScreenshot 렌더 후 링 픽셀이 focused cell 열에 있는지 단언; **버그 코드에선 FAIL(링이 day-4 열 x=96), fix 에선 PASS(day-3 x=54)** 로 검증됨. **합성(손으로 만든) 장면은 재현 실패** — 실제 grid 의 deep row→cell→glyph 중첩이 누적하는 encoder state 가 트리거라 진짜 장면이 필요(R706.1 사용자 audit 로 청산). headless 사용자 리뷰(SSOT/교과서?)가 이 gap 을 잡음.
>
> **R705.1**: (A) 수동 request_redraw → reactive `Owner::is_dirty()` bridge; (B) `pinion_runtime::rect_for_tag` scroll-blind → `Scene::rect_for_tag_absolute` 위임(a11y·라우터·앱 SSOT 통합).
>
> **R704 (datepicker)**: WAI-ARIA grid role family(Grid/GridCell/ColumnHeader/Row, Table 토대) + DatePickerExternal coordinator + 6×7 grid paint + hello-datepicker(single-Tab-stop + roving active-descendant). **주의: 이전 SEED 의 "scene/simulate RollbackFailed" 는 오진** — 실제론 demo step 8 이 `result["final"]`(없는 키)를 읽은 demo 버그였음. scene/simulate 는 final-projection snapshot node 를 직접 반환(r646 와 동일 wire). demo 정정 후 r704 PASS.
>
> 검증 3종 전부 green: cargo test 76 suites✓ / clippy `--workspace --all-targets --features pinion-runtime/vello`✓ / 전체 demo sweep 60/60✓.
>
> **R707 = 다음.** Phase B widget catalog cascade 계속 — 후보: ColorPicker(2D HSV pad + hue slider; Slider 재사용 검토) / Table(virtualization substrate-first; R704 grid-role 의 2nd consumer) / month-crossing arrow nav(R704 carry, 작음). 진입 시 class 분류 + substrate 전제 재감사. **주의: mnemosyne MCP `append_changelog_entry_v2` 가 이 환경에서 깨짐** — 존재하지 않는 CLI `append-changelog-entry-v2` 를 호출(source `/home/coin/mnemosyne` R370 은 v1 `append-changelog-entry` 만 제공); changelog 는 v1 CLI 직접 사용(audit half; --impact 는 `§5.X` 형식 — 절번호 prefix `5.` 필수, 안 그러면 orphan).
...
- R707 = **다음** — ColorPicker (2D HSV pad + hue slider, Slider 재사용 검토) / DatePicker month-crossing arrow nav 청산(R704 deferred, 작음) / Table (virtualization substrate-first; R704 grid role 이 a11y 토대 — DatePicker=grid-role 1st consumer, Table=2nd + virtualization). 진입 시 class + substrate 전제 재감사
