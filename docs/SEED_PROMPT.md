...
> **R701 land (2026-05-29, commit `a4350e2`)** — **`hello-accordion-single` = single-open WAI-ARIA APG accordion = R700 `DisclosureGroup` coordinator 의 GUI+RPC consumer** (substrate-first split 의 visible-deliverable 반쪽). **핵심**: `examples/hello-accordion-single` = hello-radio-group mirror — 단일 `DisclosureGroupExternal::new(N)` 를 composite paint root `accordion_single` 에 등록(state-scene ROOT external → RPC path `/external/<slot>`, multi-open 의 `/<tag>/external/` 와 대조). per-header row 는 `accordion_single#<i>` 로 태그 → R51.42 InputRouter `#`-split 가 header i 클릭을 coordinator 의 `invoke(send,"<i>:<EventName>")` 로 라우팅. `read_state` 는 R700 `state.<i>`/`expanded.<i>` query 로 per-section 읽음. **키보드**: per-header Tab stop (`focusable_tags()` = N composite sub-tags, RadioGroup single-roving 과 대조) → click-to-focus/`focus/set` 가 `accordion_single#<i>` 에 직접 안착, `apply_key` 가 focused sub-tag 에서 idx 파싱; Space/Enter = `<i>:KeyboardActivate` (single-open exclusion 적용); Arrow/Home/End = `focus_request` mailbox 로 focus 만 이동(R697 동형, expansion 불변). **a11y**: flat N `Button` node + aria-expanded + focused (accordion parent role 無 per ARIA), `access_child_invoke` 가 AT Click/Default 시 pointer cycle 실행; default atomic `access_focus_target` 가 정확(focus 가 composite sub-tag header 에 안착 → active-descendant redirect 불필요, hello-radio-group 과의 분기점). **검증**: 19 신규 unit test + demo `tools/demos/r701_accordion_single.py` (49 assert: single-open expanded_index 유일성 + open-switch-collapse + collapse-to-none, click/keyboard/invoke/intervene/negative 전경로). `cargo test --workspace -j2` 74 suite green(+1 example), clippy `-D pedantic` clean, r697 multi-open regression PASS. Mnemosyne `R701`(ledger 582→583; T1 +0; RT 1/1; sync; impact [5.38,5.40,5.35,5.34]). **환경**: `-j2`(CARGO_BUILD_JOBS=2) 필수(OOM).
>
> **다음 세션 진입**: `load` 단독 입력. **R702 = Phase B widget-catalog breadth 재개**. 후보: Table (Model/View + virtualization, substrate-first) / Drawer (overlay-dismiss substrate-first → menu click-outside R691 carry 동시 청산) / DatePicker / ColorPicker. 진입 시 [[round-direction-auto-select-no-ask]] 대로 northern-star 정렬 자동 선택 + 후보 substrate 전제 grep 재감사([[substrate-incompleteness-signal]] / substrate-dependency-driven widget 선택). 큰 substrate 필요하면 그 substrate 라운드 먼저(substrate-first ordering, R694 focus-ring→R695 Tooltip 선례).
>
> R701 가중 진척: Phase A 97% + Phase B 25% × ~95.5% + Phase C 35% × ~12% = 북극성 가중 **~39.1%** (single-open accordion = framework coordinator 의 visible consumer; breadth 본격 재개는 R702).
...
- R690 Tabs ✓ / R691 Menu ✓ / R692 Toolbar ✓ / R693 Dialog ✓ / R694 focus-ring substrate ✓ / R695 Tooltip ✓ / R696 Disclosure ✓ / R697 Accordion(multi-open) ✓ / R698 widget_state_name! SSOT sweep ✓ / R699 widget_event_name! from_name + parse_*_event SSOT sweep ✓ / R700 DisclosureGroup single-expand coordinator ✓ (substrate-first) / R701 hello-accordion-single ✓ (single-open accordion = R700 consumer, hello-radio-group mirror)
- R702+ = Table (Model/View, virtualization substrate-first) / Drawer (overlay-dismiss substrate-first, menu click-outside R691 carry 동시 청산) / DatePicker / ColorPicker. 진입 시 class + substrate 전제 재감사
