...
> **R691 land (2026-05-28, commit `c4b89f1`)** — Phase B widget-catalog 계속, R690 Tabs 후속. **Menu widget** (menubar + dropdown command items). **첫 감사 재확인**: menu item = **command-class** — WAI-ARIA base `menuitem` 은 `aria-selected`·`aria-checked` 둘 다 없음 (activate 시 command 발화 + dismiss; `tab`=selected / `radio`=checked 와 구분; stateful 은 별도 `menuitemcheckbox`/`menuitemradio` role) → **`RadioGroupExternal` 재사용 금지** (R690 Tabs 와 대조), 신규 command-class `MenuBarExternal`. **추가 감사 발견 (`[[verify-seed-claims-audit-first]]`)**: SEED 가 권한 "`hello-popover` popover-anchoring substrate 재사용" 은 **misnomer** — hello-popover 는 실제 dropdown-anchoring substrate 가 아니라 `IntrinsicAfterFirstPaint` window-sizing demo. 그런 substrate 부재 → dropdown 은 `LayoutStyle.absolute_position` (CSS-absolute, out-of-flow, root last-child = paint-on-top) overlay 로 직접 구현. **구현**: (1) `pinion-a11y::role` — `AriaRole::{MenuBar, Menu, MenuItem}` (accesskit `Role::{MenuBar,Menu,MenuItem}` lower + WAI-ARIA literal); `tree.rs add_actions_for_role` 에서 `MenuItem` = commit-class (Click+Focus, Button/Tab 동렬), `MenuBar`/`Menu` = focus-only container. (2) `pinion-core::widgets::menu` (신규) — `MenuBar` composite (`items_per_menu`/`open`/`active`/`bar_focus`) + `MenuBarExternal`; item activate → `"command"` intent (payload `"<menu>.<item>"`); wire = `invoke("send","t<m>:<PtrEvt>")` title toggle / `"i<i>:<PtrEvt>"` item hover·activate / `invoke("key","<W3CKey>")` 전체 WAI-ARIA §3.5 키모델 (External 이 키모델 소유, binding apply_key 는 forward). (3) `pinion-widget-paint::menu` (신규) — `MenuStyle::m3_default` + `composite_title_tag`/`composite_item_tag` + `view_menu_bar` (title strip) + `view_menu_dropdown` (absolute_position floating, fixed title slot = 결정적 x anchor). (4) `examples/hello-menu` (신규 첫 consumer) — File/Edit/View; Arrow L/R titles+wrap, Down open, Up/Down item+wrap, Home/End, R/L menu switch, Escape close, Enter/Space activate; a11y walker = MenuBar + N MenuItem titles + (open 시) Menu + M MenuItem (posinset/setsize + active-descendant). (5) `tools/demos/r691_menu.py` — ≥30 assertion (command intent 는 shell **stderr** intent log 로 검증 — `handle_tail` 의 per-dispatch drain 이 `scene/intents` poll race 를 먼저 이김). **검증**: `cargo test --workspace` all pass (core 1433 / widget-paint 296 / hello-menu 21 / a11y +6 role), clippy `-D pedantic` clean, demo sweep **47→48 PASS** regression 0, Mnemosyne `R691` (ledger 567→568; T1 new=0; RT 1/1; GENERATED sync; impact_refs [5.16, 5.38, 5.40, 5.50]). **환경 주의**: full `cargo test/build --workspace` (default -j) 가 동시 링크 스파이크로 세션 OOM-kill → `-j2` (CARGO_BUILD_JOBS=2) 로 cap 해야 안전 (스왑 15GB 사용 중인 환경).
>
> **다음 세션 진입**: `load` 단독 입력. R692 = Phase B widget catalog 계속 — Dialog / Toolbar / Table / Tooltip 중 택1 (1라운드 1위젯 또는 small pair). 진입 시 해당 위젯이 selection-class 인지 command-class 인지 먼저 감사 (Menu=command / Tabs=selection 선례).
>
> R691 가중 진척: Phase A 97% + Phase B 25% × ~85% + Phase C 35% × ~12% = 북극성 가중 **~37-38%**.
...
R691 carry (Menu widget deferred axes — 위젯 자체는 완결, 아래는 추가 axis):
- Menu **click-outside / focus-loss dismiss** = cross-widget overlay-dismiss substrate (위 cascade 참조); 현재 dismiss = Escape / 열린 title 재클릭 / item activate 로 완결
- **content-width menu titles + content-anchored dropdown** (post-view layout-cache anchor, caret 와 동형) — 현재 fixed title slot 으로 결정적 x anchor; proportional 필요 consumer 시
- `aria-haspopup`/`aria-expanded` (title), `menuitemcheckbox`/`menuitemradio` stateful, nested submenu, accelerator/mnemonic key, dropdown elevation shadow = 추가 a11y/widget axis

Phase B widget catalog cascade (R692+):
- R690 = Tabs ✓ (selection-class, RadioGroupExternal 재사용)
- R691 = Menu ✓ (command-class, 신규 MenuBarExternal)
- R692+ = Dialog / Toolbar / Table / Tooltip / Drawer / Accordion / DatePicker / ColorPicker (1라운드 1위젯 또는 small pair). 진입 시 selection-class vs command-class 먼저 감사
...
- **새 위젯 = command-class vs selection-class 먼저 분류** (R690/R691) — WAI-ARIA role 의 `aria-selected`/`aria-checked` 유무가 기준. Tabs(`tab`=selected) = selection → RadioGroupExternal 재사용; Menu(`menuitem`=무상태 command) = command → 신규 External. 잘못 분류하면 substrate 재사용/신규 결정이 틀어짐
