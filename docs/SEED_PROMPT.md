...
【R683 plan】next round entry — **4-axis paint pipeline rewrite series Round 4 of 4** (axis 1: runtime window lifecycle + Splitter widget + DockSurface + first dock consumer + R679 DevTools cascade closure). Single commit, 6-8 atomic.

(0) **runtime `Signal<Vec<WindowSpec>>` lift** — `WidgetView::windows() -> Vec<WindowSpec>` compile-time signature stays for backward-compat but bindings opt in via new `WidgetView::windows_signal(&self) -> Option<Rc<Signal<Vec<WindowSpec>>>>` (default None → use compile-time list, opt-in returns reactive list). `pinion-shell::AppShell` Effect subscribes the signal (when present) and diffs spec ids on change. `WindowSpec` itself gains `non_exhaustive` if not already so additive fields land in future rounds. 6-8 unit tests pin: compile-time path unchanged + signal-driven path adds/removes window slots on diff.

(1) **`AppShell::reconcile_windows` Effect** — on `Signal<Vec<WindowSpec>>` change: diff old vs new spec id list; spec_id added → call existing `resume_spec(&spec)` helper (R670.B substrate); spec_id removed → close winit Window + drop WindowSlot (per-slot Owner scope cleanup integrates with R680 `window_owners` map, which already lazy-creates on demand). Idempotent on identical-list re-emits. 6-8 unit tests pin the diff matrix.

(2) **`pinion-widget-paint::splitter::Splitter` widget** — `Scene::Container` wrapping 2 child panels + draggable handle child; `Signal<f32>` ratio (0.0..=1.0) controlling flex_grow split; LayoutStyle wire + DeferredInput::Drag consumer (R660 substrate primary path). M3 visual: 4px handle width + Outline color + hover state-layer. 10-12 unit tests pin: ratio mutation propagates to layout + drag updates ratio + clamping at edges + horizontal vs vertical orientation.
