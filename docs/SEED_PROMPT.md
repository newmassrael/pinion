...
> **R696 land (2026-05-29, commit `b78d0ca`)** — **Disclosure (WAI-ARIA show/hide) 위젯 + `aria-expanded` a11y 축 + `#[widget] state_flags(expanded = bool_field)` derive 확장**. 카탈로그 첫 descriptive show/hide 컨트롤(accordion 의 atomic 단위). **핵심**: (1) `Disclosure`/`DisclosureExternal` = Checkbox 와 1:1 동형 4-state 버튼-토글 statechart(`standard_button.sce-template`, `disclosure.activate`); `expanded:bool` sidecar + `"expanded"` intent(checkbox `"checked"` 와 분리 → AI 가 show/hide vs form 상태 구독 구분), introspect slot state/expanded/send. (2) `aria-expanded` = **AccessNode 필드**(`with_expanded`), `AccessState` 아님 — additive a11y 축은 selected/modal/described_by 처럼 AccessNode 에 둬야 ~20개 hand-written `AccessState{..}` a11y_manual 리터럴이 새 필드 enumerate 강제 안 받음(R674/R693/R695 convention). 처음에 AccessState 에 뒀다가 workspace test 가 todomvc 등 리터럴 break 잡아 즉시 layer 정정. tree.rs 가 accesskit `set_expanded` 로 lower. (3) `#[widget] state_flags(expanded = bool_field(N))` = checked 와 같은 2-form, 단 `AccessState` literal 이 아니라 derived `AccessNode` 에 `.with_expanded(..)` chain(value_chain 과 동형). (4) `view_disclosure` = `U+25B6`/`U+25BC` twisty + summary header(SurfaceContainerHigh state-layer ramp, checkbox 와 동일 weight) + expanded 일 때만 content panel; **tag 는 header 에**(root 아님) → content 클릭이 토글 안 함 + name enrich 가 header subtree 로 scope. (5) **hello-disclosure** = role=Button + expanded=bool_field(1), Space+Enter 둘 다 activate(APG disclosure 키보드 모델). **drop**: `AriaRole::Region` 후보 폐기 — APG 가 disclosure content 의 role=region 을 optional 로 명시 + 2nd consumer 부재([[abstraction-needs-second-consumer]]); panel 은 plain grouping container. **검증**: `cargo test --workspace` -j2 0-fail(disclosure core 14 + paint 7 + 예제 a11y 7 + a11y substrate 4), clippy `-D pedantic` clean, `r696_disclosure.py` 35-assert PASS(invoke+click+Space+Enter+intervene, aria-expanded introspect, paint panel present-only-when-expanded, disabled gate) + 전체 sweep 53/53, Mnemosyne `R696`(ledger 575→576; T1 +0; RT 1/1; GENERATED sync; impact [5.16,5.35,5.38,5.39,5.40,5.49,5.50]). **환경 주의**: `-j2`(CARGO_BUILD_JOBS=2) 필수(OOM); commit hook clippy 도 동일.
>
> **R696.A 후속 audit-clearance land (commit `d43f84b`)** — 사용자 "교과서/SSOT?" 리뷰 요청 → 자가 감사로 SSOT smell 발견 청산: DisclosureState↔SCXML-id 매핑이 3중 복제(core `disclosure_state_name` + 예제 `disclosure_state_name` + 예제 `parse_disclosure_state`)였음 — checkbox mirror 가 R643 `widget_state_name!` SSOT primitive 이전 부채를 그대로 전파. `widget_state_name!(DisclosureState, default=Idle, [...])` 선언 후 External introspect 는 `self.state().as_name()`, 예제 read_state 는 `from_name_or_default`, fmt_state_log 는 `as_name` 로 라우팅 → 매핑 1곳(매크로). behaviour-identical(문자열 동일). Disclosure 가 External+binding 양쪽을 트레잇으로 완전 라우팅한 첫 위젯. **honest cross-widget 부채 노출**: checkbox/radio/button/slider 의 External introspect 는 아직 local `*_state_name` fn 사용(button/slider 는 매크로를 예제 derive 용으로만 선언) → `widget_state_name!` adoption sweep 은 별도 라운드. 3-gate green; Mnemosyne `R696.A`(ledger 576→577, impact [5.16,5.38]).
>
> **다음 세션 진입**: `load` 단독 입력. R697 = Phase B widget catalog 계속. 후보 = **Table** (Model/View — 그러나 §5.27 VirtualList = comment-only/미구현(R696 grep 확인) → virtualization substrate 라운드 먼저, multi-round) / **Drawer** (anchored edge slide; click-outside = menu click-outside(deferred)와 일괄 설계할 overlay-dismiss substrate 필요 = 2nd consumer) / DatePicker / ColorPicker / **Accordion** (multi-section: N Disclosure via create_extra_externals + section-group + 선택적 roving). 진입 시 class(command/selection/descriptive/container) + substrate 전제 먼저 grep(R696 이 substrate-dependency-드리븐 선택의 또 한 예 — 작은 substrate 는 1st consumer 와 함께 land, 큰 substrate(virtualization/overlay-dismiss)는 substrate-first 라운드).
>
> R696 가중 진척: Phase A 97% + Phase B 25% × ~94% + Phase C 35% × ~12% = 북극성 가중 **~38.8%** (catalog widget +1 Disclosure + aria-expanded a11y 축 + #[widget] expanded derive 확장).
...
R696 carry (disclosure/accordion 잔여):
- Accordion(multi-section) = Disclosure 의 composition; N Disclosure via create_extra_externals + section-group + 선택적 roving keyboard — consumer 라운드(현재 atomic Disclosure 만).
...
- disclosure content height-collapse 애니메이션 — 현재 instant show/hide; spring height 는 measured-content clip substrate 필요(Phase B polish).
...
- R690 Tabs ✓ / R691 Menu ✓ / R692 Toolbar ✓ / R693 Dialog ✓ / R694 focus-ring substrate ✓ / R695 Tooltip ✓ / R696 Disclosure ✓ (descriptive show/hide + aria-expanded a11y 축 + #[widget] expanded=bool_field derive 확장)
- R697 = Table (Model/View, multi-round — virtualization substrate-first) / Drawer (anchored edge slide + overlay-dismiss substrate-first) / Accordion (multi-section Disclosure 스택) / DatePicker / ColorPicker. 진입 시 class(command/selection/descriptive/container) + substrate 전제 먼저 감사
