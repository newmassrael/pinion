...
> **R695 land (2026-05-29, commit `ebdd730`)** — **Tooltip 위젯 (descriptive class, WAI-ARIA `tooltip`, WCAG 1.4.13) + `scene/hover` RPC primitive + `aria-describedby`**. 카탈로그 첫 descriptive-class 위젯. **핵심**: (1) `TooltipExternal` = 자기완결 가시성 statechart (`hovered`/`focused`/`dismissed`; `visible = (hovered||focused) && !dismissed`) — anchor 가 곧 위젯이라 hover(PointerEnter/Leave) + focus(`on_focus_change`, R694 채널) + dismiss(`invoke("dismiss")`)가 직접 구동, anchor→overlay back-channel 無. dismiss latch 는 trigger-episode 하강엣지(focus 없는 leave / hover 없는 blur)에서 해제 → 재hover/focus 시 재표시. (2) `scene/hover` = click/drag/wheel/key 의 peer 인 pointer-position-only input primitive(`DeferredInput::Hover` → bare `cursor_moved`); tooltip 1차 트리거 + AI-first 의무. (3) `view_tooltip` = M3 elevated/outlined surface + `TooltipPlacement` + `anchor_position`(anchor edge flush + overflow flip + 수평 clamp, **inline 1st consumer**; Menu dropdown 은 flip 無 → 2nd consumer 아직 없음). (4) **WCAG hoverable** = overlay 가 anchor tag + `#pop` sub-index 보유 → body hover 가 같은 external 로 라우팅(transition 無, timer 無), 동시에 독립 위치조회 가능; gap 0 contiguous 필수. (5) **Escape funnel** = shell 이 Escape 를 focused widget `apply_key` 에 먼저 위임(`CoreShell::try_apply_key`), 미처리 + 비modal 시에만 quit fallback → Tooltip dismiss + Dialog cancel 단일 funnel(`handle_named_key` 도 `try_apply_key` 로 리팩터). (6) **hello-tooltip** = 2 status/feature control(autosave primary + offline extra; R695.B 에서 Save/Delete → Auto-save/Offline reframe); autosave 아래로, offline(저우측) 위로 flip + 우측 clamp; `aria-describedby` 가 각 control→Tooltip node 연결(이름 paint enrich = SSOT). **honest carry**: M3 plain-tooltip inverse-surface 팔레트(별도 palette axis — 현재 SurfaceContainerHighest valid M3 rich-tooltip tone), generic anchor→overlay 결합 primitive(2nd consumer 대기), 4dp offset(hit-bridge primitive 필요), overlay-dismiss substrate(여전히 1-consumer). **검증**: `cargo test --workspace` -j2 0-fail, clippy `-D pedantic` clean, `tools/demos/r695_tooltip.py` PASS(35+ assert: hover/hoverable/focus trigger, scene-invoke + Escape dismiss, latch reset, delete flip+clamp, 양 external scene/hover) + 전체 demo sweep 52/52, Mnemosyne `R695`(ledger 572→573; T1 +0; RT 1/1; GENERATED sync; impact_refs [5.16,5.35,5.38,5.39,5.40,5.49,5.50]). **환경 주의**: `-j2`(CARGO_BUILD_JOBS=2) 필수(OOM); commit hook clippy 도 동일.
>
> **R695.A 후속 audit-clearance land (commit `1a0054b`)** — self-review 발견 edge 청산: `TooltipExternal::dismiss` 를 `visible()` 가드 → hidden 상태 stray dismiss(raw RPC `scene/invoke dismiss`)가 latent latch 로 다음 hover/focus show 를 억제하던 edge 제거("dismiss" = "현재 표시된 tooltip 숨기기"). shell Escape 경로는 이미 visible 가드였으나 raw RPC invoke edge 를 source 에서 차단. test `dismiss_while_hidden_is_a_noop`; 3-gate green; Mnemosyne `R695.A`(ledger 573→574, impact [5.38]).
>
> **R695.B 후속 예제 honesty land** — hello-tooltip 의 control 라벨을 destructive-verb(Save/Delete)에서 status/feature affordance(Auto-save/Offline)로 reframe: tooltip-only control 이 수행 안 하는 click 동작을 암시하면 안 됨(self-review honesty 지적). 기하(rect/위치) 불변 → demo flip/clamp assertion 동일; tags autosave/offline + demo var rename. generic "실제 command widget 에 tooltip 부착"은 여전히 2nd consumer 대기. 3-gate green; Mnemosyne `R695.B`(ledger 574→575, impact [5.16]).
>
> **다음 세션 진입**: `load` 단독 입력. R696 = Phase B widget catalog 계속. 후보 = **Table** (Model/View, multi-round 큰 axis — 데이터-분리 + 대용량 virtualization; 카탈로그 첫 Model/View) / **Drawer** (anchored edge slide + overlay-dismiss) / Accordion / DatePicker / ColorPicker. 진입 시 class(command/selection/descriptive/container) + substrate 전제 먼저 grep. **신규 surface 후보**: overlay-dismiss substrate(menu click-outside + popover + combobox)가 이제 **Drawer/Popover 가 click-outside 2nd consumer**로 등장하면 menu click-outside 와 일괄 설계 가능(Tooltip 은 pointer-leave/blur dismiss 라 그 consumer 아니었음). Table 은 R32 §5.27 VirtualList spec drift(R690.A 정정) 와 연계 — 구현 전 grep 으로 실재 재확인.
>
> R695 가중 진척: Phase A 97% + Phase B 25% × ~93% + Phase C 35% × ~12% = 북극성 가중 **~38.5%** (catalog widget +1 Tooltip + scene/hover input primitive + Escape widget-first funnel).
...
Phase B widget catalog cascade (R696+):
- R690 Tabs ✓ / R691 Menu ✓ / R692 Toolbar ✓ / R693 Dialog ✓ / R694 focus-ring substrate ✓ / R695 Tooltip ✓ (descriptive class + scene/hover primitive + aria-describedby + Escape widget-first funnel)
- R696 = Table (Model/View, multi-round — 카탈로그 첫 데이터-분리 + virtualization) / Drawer (anchored edge slide + overlay-dismiss) / Accordion / DatePicker / ColorPicker. 진입 시 class(command/selection/descriptive/container) + substrate 전제 먼저 감사
