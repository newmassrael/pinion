...
- **apply_aria_activate single-External 전제** — `pinion_core::widgets::aria::apply_aria_activate` 가 `scene` 를 `Scene::External` 로 가정(단일 위젯 shape). hello-dialog 가 첫 multi-External(Container) button-activate consumer 라 `activate_focused_button` (find_external_with_tag_mut descend) inline. 2nd multi-External button consumer 시 helper 에 container-descend lift.
