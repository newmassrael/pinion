...
> **R699 land (2026-05-29, commit `4cb439c`)** — **`widget_event_name!` from_name 확장 + `parse_*_event` SSOT 청산** (R698 event-side mirror). 새 동작 0 — behaviour-preserving(audit-clearance class). **핵심**: SCE-emit Event ↔ name 매핑을 전부 R643 `WidgetEventName` primitive로 라우팅. trait 에 fallible `from_name(&str)->Option<Self>`(+ `: Sized`) 추가; macro 를 **2-group 형식**(`external = [..]` / `internal = [..]`)으로 재작성 — `as_name` 은 양 group 전체 total(internal `*Activate` raise + SCXML 3.13 `Null` 도 canonical name), `from_name` 은 external subset 만(internal/Null reject = 기존 `parse_*_event` 계약 그대로; state side 의 total `from_name_or_default` 와 의미 다름 → "default event" 없음, RPC 가 unknown 으로 statechart desync 방지). **state side 와 비대칭이라 단순 mirror 불가했던 게 R698 가 event 축 남긴 이유**(R698.A honesty correction 확정). (1) 9 SCE Event 전부 macro 채택 — button/slider 는 old flat-list 에서 migrate, checkbox/radio/toggle/disclosure/scrollbar/listbox_item/text_field 신규(variant list 는 각 위젯 generated `*_sm.rs` 에서 추출; scrollbar/slider 는 KeyboardActivate 無, text_field external = Focus/Blur/Begin·Commit·CancelEdit/Disable/Enable). 9 `parse_*_event` 제거; 모든 `invoke('send',name)` 사이트가 `Event::from_name` 호출(cross-module radio_group→RadioEvent, listbox→ListboxItemEvent 포함, pub(crate) parse + use-import 청산); 위젯별 test 가 external round-trip + internal/Null/unknown reject + as_name totality 검증. (2) 14 example 의 forward `event_name` 을 `as_name` SSOT 로 라우팅(`pinion_core::WidgetEventName::as_name(&event)`, `_ => "__internal__"` table 제거) — hello-radio/-checkbox/-toggle/-disclosure/-slider-vertical/-textfield/-theme/-commands/-button-tui/-textfield-tui/-toggle-tui/-commands-tui + settings-panel + todomvc(4 subagent 병렬 edit-only, 단일 빌드 검증). `()`-stub event_name(menu/toolbar/tooltip/dialog/tabs/accordion/listbox*/radio-group)은 SCE event 없어 불변; figma-button-m3 는 `state_name_derive`(combined flag→event_name derive 포함)로 이미 routed(stale 주석만 정정). forward `event_name` 은 `ShellCore::forward` 가 external event 만 전달하므로 제거된 `__internal__` catch-all 은 unreachable(dock-panels/tree-view/multi-window 가 이미 그 패턴 증명). **검증**: `cargo test --workspace` -j2 73 suite green 0-fail(core 1502, R698 1501 대비 +1), clippy `-D pedantic` clean, RPC sweep(settings_panel_r669 20-assert + todomvc_r666 12+step + hello_button/-toggle/-listbox + r696_disclosure + r697_accordion) PASS — `from_name`(invoke send) + `as_name`(introspect) 양경로 byte-identical(touched release bin 사전 재빌드). Mnemosyne `R699`(ledger 580→581; T1 +0; RT 1/1; GENERATED sync; impact [5.16]). **환경**: `-j2`(CARGO_BUILD_JOBS=2) 필수(OOM); commit 도 `CARGO_BUILD_JOBS=2 git commit`.
>
> **다음 세션 진입**: `load` 단독 입력. R700 = **Phase B 진입 마일스톤** — widget catalog 계속. 후보 = **Drawer**(anchored edge slide + overlay-dismiss substrate-first; menu click-outside 와 일괄 설계 = overlay-dismiss 2nd consumer) / **Table**(Model/View, multi-round — virtualization substrate-first, §5.27 VirtualList 은 comment-only) / **Accordion single-open variant**(RadioGroup-mould mutual-exclusion substrate) / DatePicker / ColorPicker. 진입 시 class(command/selection/descriptive/container) + substrate 전제 먼저 grep(작은 substrate 는 1st consumer 와 함께, 큰 substrate 는 substrate-first 라운드).
>
> R699 가중 진척: Phase A 97% + Phase B 25% × ~95% + Phase C 35% × ~12% = 북극성 가중 **~38.9%** (SSOT cleanup — 신규 widget 0, 신규 substrate 0; quality 축, breadth 무변. R700 부터 Phase B breadth 재개).
...
- R690 Tabs ✓ / R691 Menu ✓ / R692 Toolbar ✓ / R693 Dialog ✓ / R694 focus-ring substrate ✓ / R695 Tooltip ✓ / R696 Disclosure ✓ / R697 Accordion ✓ / R698 widget_state_name! SSOT sweep ✓ / R699 widget_event_name! from_name + parse_*_event SSOT sweep ✓ (event-id↔enum 매핑 전부 WidgetEventName 라우팅; behaviour-preserving, 신규 widget 0)
- R700 (R699 후 감사로 설계 확정) = **single-open accordion via `DisclosureGroup` coordinator** 1순위. 정통 근거: R47-class — single-expand mutual exclusion 은 framework-owned(RadioGroup 선례), application inline 금지. **설계**: `crates/pinion-core/src/widgets/disclosure_group.rs` 를 `radio_group.rs`(1215 LOC) **mirror** — `DisclosureGroup { disclosures: Vec<Disclosure>, expanded: Option<usize> }` (RadioGroup 과 달리 `focused_index` 不要 — R697 가 헤더별 Tab stop + arrow-roving 을 이미 `focusable_tags`+`focus_request` 로 처리; coordinator 는 expand-exclusion 만 소유). `send(i, DisclosureEvent)`: disclosure[i] drive 후 `is_expanded(i)` 면 sibling 전부 `set_expanded(false)`+`expanded=Some(i)`, collapse edge 면 `expanded=None`. **prereq 확인됨**: `Disclosure::set_expanded(bool)`(disclosure.rs:135) 존재 = sibling-collapse substrate 준비. `WidgetTransition`: Event=`(usize,DisclosureEvent)`, Snapshot=`Option<usize>`, detect 가 변화 시 `"expanded"` intent(Int index) emit. `DisclosureGroupExternal` + ExternalIntrospect(query "count"/"expanded_index", intervene "expanded_index", invoke "<idx>:<EventName>") 도 radio_group mirror. 그 후 example 소비자(hello-accordion single-open variant 또는 새 crate) + RPC demo ≥30 assert. **규모 큼**(core+example+demo) → 한 세션 완주 가능하나 substrate-first split(R700 core+unit / R701 consumer+GUI)도 정통(RadioGroup R51.15 가 logical group 먼저 land 한 선례). 대안 후보: Drawer(overlay-dismiss substrate-first, menu click-outside R691 carry 동시 청산) / Table(virtualization) / DatePicker / ColorPicker. 진입 시 class + substrate 전제 재감사
