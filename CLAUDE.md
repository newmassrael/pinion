...
| **Pro-tool performance** | 9 | **R1744** re-judged and the number **HELD at 90** — the tool demands a look, not a move. All five R1558 items are exactly where they were: **present latency** stays external (wgpu exposes no presentation-timestamp extension); the footprint is what the allocator was *asked for*, not what is resident; per-node replay status is absent by construction; a profile row's address has no general reader; R1550's two modelled arenas are unpinned. ★**The look's real finding is why it did not go up**: R1705, R1708 and R1723 each fixed a cost this list had **never named** (no repeating fill — 95,131 nodes for a canvas grid; 81 blocking paints for one resize drag; a synchronous round trip per window per pointer event), each reported by a person using the tool. A tree with those three properties was not at 90%, so the number was overstated before and those rounds brought reality up to it. ⇒ the axis now carries a **new largest item: it cannot state that a repeated or superseded unit of work is cheap** — three answers written three times, no fourth caller, nobody auditing the cost model |
