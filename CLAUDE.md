...
| **B. Professional GUI** (the toolkit / another retained-mode toolkit / another declarative toolkit / web UI library-class) | Multi-window + DCC/IDE/CAD-grade widget catalog + pro-tool performance | **~89%** all axes / **~93%** buildable — per-axis detail below |
...
advanced from here, so 100% of all-axes is unreachable by construction. **~93%
...
| **Pro-tool performance** | 9 | **R1756** re-judged 90 → **88** — the project's **third downward move** (after R1577, R1604), and for their reason: a measurement finally replaced an assumption, so the meter is what is new. ★★★★★**The axis's HEADLINE INSTRUMENT measures the wrong thing.** `render_us` is documented "record + submit, CPU-side" and the shell subtracts the acquire so it can be called *work only* — but it also brackets `present()`, which **blocks on the consumer**. Measured R1754, one app, one scene, 133 draws, two displays of one machine: `mean_render_us` **10,384 µs vs 997,132 µs** while `gpu_us` was 491 / 1,759 µs. 133 draws record and submit in microseconds ⇒ **99.8% of that field is neither.** This is R1361.1's defect at the frame's other end — that round split the *acquire* block out and left the *present* block in ⇒ [[debt-render-us-bills-a-blocking-present-as-work]], now the axis's **largest item**. What landed does not offset it: R1752's `captured`/`captured_frames` and R1754's `adapter` are real instrument capability, but *a field that cannot say what it measured is a smaller defect than one that measures the wrong thing*. R1748/R1749 (TextGrid per-cell rect + glyph call, consumer-reported 4 fps) are **not credited** — R1744's rule applied a second time: repayment restores credit rather than adding it. The prior six items are all unmoved: present latency external; footprint = what the allocator was *asked for*; per-node replay absent by construction; a profile row's address has no general reader; R1550's two arenas unpinned; R1744's repeated-work cost model unaudited |
