...
| **B. Professional GUI** (Qt / Flutter / Compose / React-class) | Multi-window + DCC/IDE/CAD-grade widget catalog + pro-tool performance | **~86%** all axes / **~91%** buildable — per-axis detail below |
...
advanced from here, so 100% of all-axes is unreachable by construction. **~91%
...
|---|--:|---|
| **Advanced DCC / IDE widgets** | 20 | **R1604** re-judged 98 → 86 — the largest move here and the second downward one. The node-graph third now has a *measured, test-backed* census where the 98 rested on a hand count: **Blender 54/72 = 75%**, **Unreal 60/149 = 40%**, so node graph 97 → 62 beside property grid ~98 / data grid ~98. **Nothing was lost — the meter is new.** Open (R1594): ~7 editor gestures; **execution semantics** (no Blender comparison will surface these — it is pure dataflow too); `hello-node-editor` still holds its own model ([[debt-two-node-graph-models]]) |
| **Model/View at scale** | 16 | **R1563**: the section axis answers 2 of Qt's 4 header roles; **drag-select across sections** is blocked on the pointer wire not reporting held buttons (W3C `PointerEvent.buttons`); the keyboard has no two-axis vocabulary (`Ctrl+Space`, `Ctrl+Shift+Arrow`); `SelectColumns` has no binding; the eager `Table` keeps its own rectangle cell-selection, so the tree carries **two** cell-selection models |
| **Common widget catalog** | 16 | **R1554**: wheel on `QComboBox` / `QTabBar` is the largest cross-cutting item (`External::wheel` still has exactly two implementors); mnemonic adoption is 4 sites; five absent widget kinds (`QDial`, a paged container, `QKeySequenceEdit`, `QFontComboBox`, canned `QMessageBox` / `QInputDialog`); the disabled cascade has one consumer |
| **Charting / dataviz** | 10 | **R1568**: three series types left (spline, 3D-surface → Phase C's renderer, and the **OHLC bar**, now the cheapest item); no per-mark a11y on box / candle / polar; polar has no cross-filter leg and no legend interaction; `QCategoryAxis`, label thinning and local time all still absent |
| **Rich-text editing/selection** | 9 | **R1560**: `setMarkdown` / `toHtml` is the last of R1551's three; nested tables need the general `QTextFrame` containment axis, not a second level counter; `QTextBlockFormat` keeps 3 untaken properties; the character half is unchanged since R1546 (vertical alignment, overline) |
...
| **OS-native maturity** *(gated)* | 11 | **R1576**: a display's **usable region** (Qt `availableGeometry`) needs a platform probe — winit's `MonitorHandle` has no work-area accessor and `_NET_WORKAREA` is one rect for the whole desktop; no hot-plug event (winit 0.30 emits none). What this axis is judged short on is Mac/Win native surfaces, **untouched** |
| **§7 API stabilisation** *(gated)* | 9 | **R1566**: the declaration gate is still per widget; ~99 schema-declaring bindings were never probed; **`SchemaChannel` cannot say a slot is WRITABLE** (Qt's `QMetaProperty::isWritable()` can). All four things "stabilisation" names — method→type binding, version negotiation, deprecation path, freeze — still stand |
