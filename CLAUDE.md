...
| **B. Professional GUI** (the toolkit / another retained-mode toolkit / another declarative toolkit / web UI library-class) | Multi-window + DCC/IDE/CAD-grade widget catalog + pro-tool performance | **~86%** all axes / **~91%** buildable — per-axis detail below |
...
| **Common widget catalog** | 16 | **R1554**: wheel on combo box / tab bar is the largest cross-cutting item (`External::wheel` still has exactly two implementors); mnemonic adoption is 4 sites; five absent widget kinds (dial, a paged container, key-sequence editor, font picker, canned message box / input dialog); the disabled cascade has one consumer |
...
| **Rich-text editing/selection** | 9 | **R1560**: `setMarkdown` / `toHtml` is the last of R1551's three; nested tables need the general text frame containment axis, not a second level counter; text block format keeps 3 untaken properties; the character half is unchanged since R1546 (vertical alignment, overline) |
...
| **§7 API stabilisation** *(gated)* | 9 | **R1566**: the declaration gate is still per widget; ~99 schema-declaring bindings were never probed; **`SchemaChannel` cannot say a slot is WRITABLE** (the toolkit's `isWritable()` can). All four things "stabilisation" names — method→type binding, version negotiation, deprecation path, freeze — still stand |
