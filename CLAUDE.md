...
> **1. Model/View at scale (400) · 2. Pro-tool performance (360) · 3. Charting (350) · 4. Advanced DCC (300) · 5. Common catalog (288) · 6. Rich-text (270)**

**Do not hand-edit that line — re-run the tool.** It already changed once: at R1519 perf led with 450, because it was the axis that had moved least (43% → 50% in 587 rounds). R1520 and R1521 then optimised it and R1522 re-judged it to 60%, which drops it to second behind Model/View. That is the mechanism working — the order is a function of the completions, so it moves when they do. Take the tool's order; the numbered sections below are the per-axis DETAIL, not the priority. Audit-first within each:
