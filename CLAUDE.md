...
> **1. Model/View at scale (320) = Charting (320) — a genuine tie · 3. Pro-tool performance (315) · 4. Advanced DCC (300) · 5. Common catalog (288) · 6. Rich-text (270)**

**Do not hand-edit that line — re-run the tool.** It has already changed three times: at R1519 perf led with 450, being the axis that had moved least (43% → 50% in 587 rounds); R1520 and R1521 optimised it and R1522 re-judged it to 60%, dropping it behind Model/View; then R1523-R1525 deepened Model/View and R1526 re-judged that to 80%, putting perf back on top; then R1527 optimised perf and re-judged it to 65%, so **charting now leads** — the axis nothing has touched since R1442. That is the mechanism working — the order is a function of the completions, so it moves when they do. Note what the re-judgments needed first: an evidence kind that could *see* the work. R1522 and R1526 were both forced by an axis registering **+0%** while real work landed on it; R1527 is the first re-judgment the tool **demanded on its own** — it exited STALE because the round it had just registered moved perf's evidence past the 25% band. Take the tool's order; the numbered sections below are the per-axis DETAIL, not the priority. Audit-first within each:
...
**7. Charting / data visualisation — ~68% (R1528 re-judged), THE AXIS THAT DID NOT EXIST.** `pinion-chart` + `pinion-graph`, 22 examples, 72 demos (R1372-R1442): line/scatter/donut/treemap/heatmap/histogram + brushing, cross-filter, linked legend, continuous colour scales with WCAG lift, Brandes-Köpf graph layout. Qt ships QtCharts, so this is Phase B under [[qt-parity-over-yagni]] — but the R931 axis set had no slot for it, so ~15% of the tree could not register as progress. R1528 added the crate's first non-affine **axis** — a logarithmic value axis on both cartesian axes of the two numeric-x charts (Qt `QLogValueAxis`), with decade snapping, per-decade minor gridlines, and off-scale samples *reported* rather than drawn on the baseline. That re-judgment is worth more for what it corrected than for the three points: R1519 named this axis's remaining gap as series types and interaction depth and **never mentioned axis types at all**, so naming that dimension reveals more absent surface than R1528 filled. Remaining vs QtCharts: **no datetime axis** (every monitoring chart has a time axis; x is a bare number today), no category axis outside the bar chart's slots, no polar / candlestick / box-plot / spline / 3D-surface series, and no plot-level zoom or pan.
