...
4. **OS-native maturity** — finish file-dialog / print / drag-drop / tray. ~48% (file-dialog / clipboard / drag-drop / prefers-color-scheme + **scene→vector PDF render [R908] + vector-PDF print spool [R911] + CUPS spool [R833] all Linux-verified** done; native-menu rendered-not-native [Mac NSMenu / Win HMENU need those OSes], no Win/Mac native print dialog, **tray 0% but NOT blocked — R932.2 decided Linux tray = pure D-Bus/ksni (StatusNotifierItem, no gtk; sidesteps the gtk-tray-crate/winit incompat), verified headlessly via a session-bus test-watcher (SNI register + dbusmenu export, ZERO-FLAKE — no panel pixels needed); see `docs/cross-platform-native-strategy.md` staged-path step ①**). The remaining real blocker is Mac/Win-specific native surfaces (need those OSes' CI runners), NOT Linux tray (doable + verifiable now) — PDF/print/file-dialog/clipboard/DnD all Linux-native + verified.
